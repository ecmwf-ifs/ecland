...
INTEGER(KIND=JPIM)              :: VARID, VARIDA, VARIDX, VARIDY
...
!** input matrix area
ALLOCATE( D2TMP(NX,NY) )
ALLOCATE( I2TMP(NX,NY) )

CALL NCERROR ( NF90_INQ_VARID(NCID,'inpa',VARIDA),'getting id' )
CALL NCERROR ( NF90_INQ_VARID(NCID,'inpx',VARIDX),'getting id' )
CALL NCERROR ( NF90_INQ_VARID(NCID,'inpy',VARIDY),'getting id' )

DO INPI=1, INPN
  CALL NCERROR ( NF90_GET_VAR(NCID,VARIDA,D2TMP,(/1,1,INPI/),(/NX,NY,1/)),'reading data' ) 
  CALL mapD2vecD(D2TMP(:,:),INPA(:,INPI))

  CALL NCERROR ( NF90_GET_VAR(NCID,VARIDX,I2TMP,(/1,1,INPI/),(/NX,NY,1/)),'reading data' ) 
  CALL mapI2vecI(I2TMP(:,:),INPX(:,INPI))

  CALL NCERROR ( NF90_GET_VAR(NCID,VARIDY,I2TMP,(/1,1,INPI/),(/NX,NY,1/)),'reading data' ) 
  CALL mapI2vecI(I2TMP(:,:),INPY(:,INPI))
END DO
...
!    WRITE(LOGNAM,*)'INIT_MAP: inpaI:',TRIM(CINPMAT)
    CALL NCERROR ( NF90_INQ_VARID(NCID,'inpaI',VARIDA),'getting id' )
    CALL NCERROR ( NF90_INQ_VARID(NCID,'inpxI',VARIDX),'getting id' )
    CALL NCERROR ( NF90_INQ_VARID(NCID,'inpyI',VARIDY),'getting id' )
    DO INPI=1, INPNI
      CALL NCERROR ( NF90_GET_VAR(NCID,VARIDA,INPAI(:,:,INPI),(/1,1,INPI/),(/NXIN,NYIN,1/)),'reading data' ) 
      CALL NCERROR ( NF90_GET_VAR(NCID,VARIDX,INPXI(:,:,INPI),(/1,1,INPI/),(/NXIN,NYIN,1/)),'reading data' ) 
      CALL NCERROR ( NF90_GET_VAR(NCID,VARIDY,INPYI(:,:,INPI),(/1,1,INPI/),(/NXIN,NYIN,1/)),'reading data' )
    ENDDO
