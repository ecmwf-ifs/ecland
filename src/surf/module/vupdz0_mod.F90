...
ASSOCIATE(RCPD=>YDCST%RCPD, RD=>YDCST%RD, RETV=>YDCST%RETV, RG=>YDCST%RG, &
 ...
 & RURBZTM=>YDURB%RURBZTM, RURBZTH=>YDURB%RURBZTH, RBLENDSNVEG=>YDEXC%RBLENDSNVEG, &
 & RCDFC=>YDEXC%RCDFC)
...
!       - Sea ice
    ...
    ENDIF
!       - Wet skin
    PZ0MTI(JL,3)=PCVL(JL)*PSSDP2(JL,SSDP2D_ID%NRVZ0ML2D)+PCVH(JL)*PSSDP2(JL,SSDP2D_ID%NRVZ0MH2D)&
      & +(1.-PCVL(JL)-PCVH(JL))*RVZ0M_BARE
!       - Low Vegetation
    PZ0MTI(JL,4)=PSSDP2(JL,SSDP2D_ID%NRVZ0ML2D)
!       - Exposed snow
! blend between roughness of low vegetation and soil and roughness of ice caps, when snow depth below 25cm 
    ZMLOW=PCVL(JL)/(1.0_JPRB-PCVH(JL))*PSSDP2(JL,SSDP2D_ID%NRVZ0ML2D)+(1.0_JPRB-PCVL(JL)-PCVH(JL))/(1.0_JPRB-PCVH(JL))*RVZ0M_BARE
...
!       - High vegetation
    PZ0MTI(JL,6)=PSSDP2(JL,SSDP2D_ID%NRVZ0MH2D)
!       - Sheltered snow
    PZ0MTI(JL,7)=PSSDP2(JL,SSDP2D_ID%NRVZ0MH2D)
!       - Bare soil
...
!       - URBAN
    IF (LEURBAN) THEN
      ...
      PZ0MTI(JL,3)=(ZPCVL*PSSDP2(JL,SSDP2D_ID%NRVZ0ML2D)+ZPCVH*PSSDP2(JL,SSDP2D_ID%NRVZ0MH2D)&
      & +ZPCVB*RVZ0M_BARE+RURBZTM*ZURBF)
      ZMLOW=ZPCVL/(1.0_JPRB-ZPCVH)*PSSDP2(JL,SSDP2D_ID%NRVZ0ML2D)+ZPCVB/(1.0_JPRB-ZPCVH)*RVZ0M_BARE
...
!   - Wet skin
DO JL=KIDIA,KFDIA
  PZ0MTI(JL,JTILE)=PCVL(JL)*PSSDP2(JL,SSDP2D_ID%NRVZ0ML2D)+PCVH(JL)*PSSDP2(JL,SSDP2D_ID%NRVZ0MH2D)+(1.-PCVL(JL)-PCVH(JL))*RVZ0M_BARE
  PZ0HTI(JL,JTILE)=PCVL(JL)*PSSDP2(JL,SSDP2D_ID%NRVZ0HL2D)+PCVH(JL)*PSSDP2(JL,SSDP2D_ID%NRVZ0HH2D)+(1.-PCVL(JL)-PCVH(JL))*RVZ0H_BARE
...
!   - Low Vegetation
DO JL=KIDIA,KFDIA
  PZ0MTI(JL,JTILE)=PSSDP2(JL,SSDP2D_ID%NRVZ0ML2D)
  PZ0HTI(JL,JTILE)=PSSDP2(JL,SSDP2D_ID%NRVZ0HL2D)
  PZ0QTI(JL,JTILE)=PZ0HTI(JL,JTILE)
ENDDO
...
DO JL=KIDIA,KFDIA
  ZMLOW=PCVL(JL)/(1.0_JPRB-PCVH(JL))*PSSDP2(JL,SSDP2D_ID%NRVZ0ML2D)+(1.0_JPRB-PCVL(JL)-PCVH(JL))/(1.0_JPRB-PCVH(JL))*RVZ0M_BARE
  ZHLOW=PCVL(JL)/(1.0_JPRB-PCVH(JL))*PSSDP2(JL,SSDP2D_ID%NRVZ0HL2D)+(1.0_JPRB-PCVL(JL)-PCVH(JL))/(1.0_JPRB-PCVH(JL))*RVZ0H_BARE
  ...
  ! The actual z0h is a log-average of vegetation and snow, dependending on ZSNWGHT
    ...
    IF (LEURBAN) THEN
      ...
      IF (ZURBF.GT.0.0_JPRB) THEN
       ZMLOW=ZPCVL/(1.0_JPRB-ZPCVH)*PSSDP2(JL,SSDP2D_ID%NRVZ0ML2D)+ZPCVB/(1.0_JPRB-ZPCVH)*RVZ0M_BARE
       ZHLOW=ZPCVL/(1.0_JPRB-ZPCVH)*PSSDP2(JL,SSDP2D_ID%NRVZ0HL2D)+ZPCVB/(1.0_JPRB-ZPCVH)*RVZ0H_BARE
...
!   - High vegetation
DO JL=KIDIA,KFDIA
  PZ0MTI(JL,JTILE)=PSSDP2(JL,SSDP2D_ID%NRVZ0MH2D)
  PZ0HTI(JL,JTILE)=PSSDP2(JL,SSDP2D_ID%NRVZ0HH2D)
  PZ0QTI(JL,JTILE)=PZ0HTI(JL,JTILE)
ENDDO
