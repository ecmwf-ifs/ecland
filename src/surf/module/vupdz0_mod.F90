...
ASSOCIATE(RCPD=>YDCST%RCPD, RD=>YDCST%RD, RETV=>YDCST%RETV, RG=>YDCST%RG, &
 ...
 & RURBZTM=>YDURB%RURBZTM,RURBZTH=>YDURB%RURBZTH, RVZ0HH2D=>PSSDP2(:,SSDP2D_ID%S2_RVZ0HH2D), &
+ & RVZ0HL2D=>PSSDP2(:,SSDP2D_ID%S2_RVZ0HL2D), RVZ0MH2D=>PSSDP2(:,SSDP2D_ID%S2_RVZ0MH2D), &
+ & RVZ0ML2D=>PSSDP2(:,SSDP2D_ID%S2_RVZ0ML2D))
...
! blend between roughness of low vegetation and soil and roughness of ice caps, when snow depth below 25cm 
   ZMLOW=PCVL(JL)/(1.0_JPRB-PCVH(JL))*RVZ0ML2D(JL)+(1.0_JPRB-PCVL(JL)-PCVH(JL))/(1.0_JPRB-PCVH(JL))*RVZ0M(0)
...
!   - Wet skin
DO JL=KIDIA,KFDIA
  PZ0MTI(JL,JTILE)=PCVL(JL)*RVZ0ML2D(JL)+PCVH(JL)*RVZ0MH2D(JL)+(1.-PCVL(JL)-PCVH(JL))*RVZ0M(0)
  PZ0HTI(JL,JTILE)=PCVL(JL)*RVZ0HL2D(JL)+PCVH(JL)*RVZ0HH2D(JL)+(1.-PCVL(JL)-PCVH(JL))*RVZ0H(0)
...
DO JL=KIDIA,KFDIA
  ZMLOW=PCVL(JL)/(1.0_JPRB-PCVH(JL))*RVZ0ML2D(JL)+(1.0_JPRB-PCVL(JL)-PCVH(JL))/(1.0_JPRB-PCVH(JL))*RVZ0M(0)
  ZHLOW=PCVL(JL)/(1.0_JPRB-PCVH(JL))*RVZ0HL2D(JL)+(1.0_JPRB-PCVL(JL)-PCVH(JL))/(1.0_JPRB-PCVH(JL))*RVZ0H(0)
...
DO JL=KIDIA,KFDIA
  ...
  PZ0QTI(JL,JTILE)=PZ0HTI(JL,JTILE)
   IF (KTILES .GT. 9 .AND. PCVH(JL)+ZURBF .GT. 0.0_JPRB) THEN
    PZ0MTI(JL,JTILE)=(RVZ0MH2D(JL)*PCVH(JL)+RURBZTM*ZURBF)/(PCVH(JL)+ZURBF)
    PZ0HTI(JL,JTILE)=(RVZ0HH2D(JL)*PCVH(JL)+RURBZTH*ZURBF)/(PCVH(JL)+ZURBF)
    PZ0QTI(JL,JTILE)=PZ0HTI(JL,JTILE)
   ENDIF
ENDDO
