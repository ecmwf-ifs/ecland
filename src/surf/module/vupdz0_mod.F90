...
SUBROUTINE VUPDZ0(KIDIA,KFDIA,KLON,KTILES,KSTEP,CDCONF,&
 ...
 & PTMLEV,PQMLEV,PAPHMS,PGEOMLEV,PRPLRG,PDSN,&
...
REAL(KIND=JPRB) :: ZBLENDZ0, ZCBLENDM, ZCBLENDH, ZCBLENDQ
...
!   Log Z averaging of low vegetation high vegetation and bare soil 
    ZCBLENDM=PCVL(JL)/(LOG(ZBLENDZ0/RVZ0M(KTVL(JL))))**2+&
           & PCVH(JL)/(LOG(ZBLENDZ0/RVZ0M(KTVH(JL))))**2+&
	   & (1.-PCVL(JL)-PCVH(JL))/(LOG(ZBLENDZ0/RVZ0M(0)))**2
    PZ0MTI(JL,3)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDM)) 
!       - Low Vegetation
...
!   Log Z averaging of low vegetation and soil to define ZMLOW then log Z average with the snow roughness using ZSNWGHT
    ZCBLENDM=PCVL(JL)/(1.0_JPRB-PCVH(JL))/(LOG(ZBLENDZ0/RVZ0M(KTVL(JL))))**2+&
	     & ((1.0_JPRB-PCVL(JL)-PCVH(JL))/(1.0_JPRB-PCVH(JL)))/(LOG(ZBLENDZ0/RVZ0M(0)))**2
    ZMLOW=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDM))
!    PZ0MTI(JL,5)=ZSNWGHT(JL)*RVZ0M(12)+(1.0_JPRB-ZSNWGHT(JL))*ZMLOW
    ZCBLENDM=ZSNWGHT(JL)/(LOG(ZBLENDZ0/RVZ0M(12)))**2+&
	     & (1.0_JPRB-ZSNWGHT(JL))/(LOG(ZBLENDZ0/ZMLOW))**2
    PZ0MTI(JL,5)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDM)) 
...
!   Log Z averaging of water and ice roughness for partially frozen lakes
      ZCBLENDM=ZLWAT(JL)/(LOG(ZBLENDZ0/PZ0MTI(JL,1)))**2+&
	     & ZLICE(JL)/(LOG(ZBLENDZ0/PZ0MTI(JL,2)))**2
      PZ0MTI(JL,9)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDM))  
    ENDIF
!       - URBAN
    IF (LEURBAN) THEN
      ZURBF=PCUR(JL)
...
!     - wet skin
      ZCBLENDM=ZPCVL/(LOG(ZBLENDZ0/RVZ0M(KTVL(JL))))**2+&
          & ZPCVH/(LOG(ZBLENDZ0/RVZ0M(KTVH(JL))))**2+&
          & ZPCVB/(LOG(ZBLENDZ0/RVZ0M(0)))**2+&
          & ZURBF/(LOG(ZBLENDZ0/RURBZTM))**2
      PZ0MTI(JL,3)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDM))
!     - Exposed snow
      ZCBLENDM=(ZPCVL/(1.0_JPRB-ZPCVH))/(LOG(ZBLENDZ0/RVZ0M(KTVL(JL))))**2+&
         & (ZPCVB/(1.0_JPRB-ZPCVH))/(LOG(ZBLENDZ0/RVZ0M(0)))**2+&
         & (ZURBF/(1.0_JPRB-ZPCVH))/(LOG(ZBLENDZ0/RURBZTM))**2
      ZMLOW=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDM))
      ZCBLENDM=ZSNWGHT(JL)/(LOG(ZBLENDZ0/RVZ0M(12)))**2+&
         & (1.0_JPRB-ZSNWGHT(JL))/(LOG(ZBLENDZ0/ZMLOW))**2
      PZ0MTI(JL,5)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDM))
...
!   Log Z averaging of low vegetation high vegetation and bare soil 
  ZCBLENDM=PCVL(JL)/(LOG(ZBLENDZ0/RVZ0M(KTVL(JL))))**2+&
         & PCVH(JL)/(LOG(ZBLENDZ0/RVZ0M(KTVH(JL))))**2+&
	 & (1.-PCVL(JL)-PCVH(JL))/(LOG(ZBLENDZ0/RVZ0M(0)))**2
  ZCBLENDH=PCVL(JL)/(LOG(ZBLENDZ0/RVZ0H(KTVL(JL))))**2+&
         & PCVH(JL)/(LOG(ZBLENDZ0/RVZ0H(KTVH(JL))))**2+&
	 & (1.-PCVL(JL)-PCVH(JL))/(LOG(ZBLENDZ0/RVZ0H(0)))**2
  PZ0MTI(JL,JTILE)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDM))
  PZ0HTI(JL,JTILE)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDH))
...
!  Log Z averaging of low vegetation and soil to define ZMLOW then log Z average with the snow roughness using ZSNWGHT
  ...
  ! high vegetation and bare soil contribution to log-average
  ZCBLENDH=PCVL(JL)/(1.0_JPRB-PCVH(JL))/(LOG(ZBLENDZ0/RVZ0H(KTVL(JL))))**2+&
         & ((1.0_JPRB-PCVL(JL)-PCVH(JL))/(1.0_JPRB-PCVH(JL)))/(LOG(ZBLENDZ0/RVZ0H(0)))**2
  ZHLOW=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDH))

  !*ZCBLENDH=ZSNWGHT(JL)/(LOG(ZBLENDZ0/RVZ0H(12)))**2+&
       !* & (1.0_JPRB-ZSNWGHT(JL))/(LOG(ZBLENDZ0/ZHLOW))**2      
  ...
  ZCBLENDQ=ZSNWGHT(JL)/(LOG(ZBLENDZ0/ZZ0QSNOW))**2+&
        & (1.0_JPRB-ZSNWGHT(JL))/(LOG(ZBLENDZ0/ZHLOW))**2           

  PZ0MTI(JL,JTILE)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDM))
  PZ0HTI(JL,JTILE)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDH))
  !*PZ0QTI(JL,JTILE)=PZ0HTI(JL,JTILE)
  PZ0QTI(JL,JTILE)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDQ))
ENDDO
...
!    PZ0QTI(JL,JTILE)=PZ0QTI(JL,1)*ZLWAT(JL)+PZ0QTI(JL,2)*ZLICE(JL)
    ZCBLENDM=ZLWAT(JL)/(LOG(ZBLENDZ0/PZ0MTI(JL,1)))**2+&
           & ZLICE(JL)/(LOG(ZBLENDZ0/PZ0MTI(JL,2)))**2
    ZCBLENDH=ZLWAT(JL)/(LOG(ZBLENDZ0/PZ0HTI(JL,1)))**2+&
	   & ZLICE(JL)/(LOG(ZBLENDZ0/PZ0HTI(JL,2)))**2
    ZCBLENDQ=ZLWAT(JL)/(LOG(ZBLENDZ0/PZ0QTI(JL,1)))**2+&
	   & ZLICE(JL)/(LOG(ZBLENDZ0/PZ0QTI(JL,2)))**2
    PZ0MTI(JL,JTILE)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDM))
    PZ0HTI(JL,JTILE)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDH))
    PZ0QTI(JL,JTILE)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDQ))
  ENDDO 
...
IF (LEURBAN) THEN
 ...
 ! ---- wet skin
    ZCBLENDM=ZPCVL/(LOG(ZBLENDZ0/RVZ0M(KTVL(JL))))**2+&
      ...
      & ZPCVB/(LOG(ZBLENDZ0/RVZ0M(0)))**2+&
      & ZURBF/(LOG(ZBLENDZ0/RURBZTM))**2
    ZCBLENDH=ZPCVL/(LOG(ZBLENDZ0/RVZ0H(KTVL(JL))))**2+&
      & ZPCVH/(LOG(ZBLENDZ0/RVZ0H(KTVH(JL))))**2+&
      & ZPCVB/(LOG(ZBLENDZ0/RVZ0H(0)))**2+&
      & ZURBF/(LOG(ZBLENDZ0/RURBZTH))**2
    PZ0MTI(JL,3)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDM))
    PZ0HTI(JL,3)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDH))
    PZ0QTI(JL,3)=PZ0HTI(JL,3)
 ...
 ! ---- exposed snow
  ...
  ! contribution to log-average
    ZCBLENDH=(ZPCVL/(1.0_JPRB-ZPCVH))/(LOG(ZBLENDZ0/RVZ0H(KTVL(JL))))**2+&
         & (ZPCVB/(1.0_JPRB-ZPCVH))/(LOG(ZBLENDZ0/RVZ0H(0)))**2+&
         & (ZURBF/(1.0_JPRB-ZPCVH))/(LOG(ZBLENDZ0/RURBZTM))**2
    ZHLOW=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDH))

    Rstar=(REAL(RVZ0M(12)*ZUST(JL,5),KIND=JPRD))/(1.35E-5_JPRD)
    ZZ0HSNOW=REAL(RVZ0M(12),KIND=JPRD)*exp( zat_sn*log(Rstar)**3._JPRB + zbt_sn*log(Rstar)**2._JPRB + zct_sn*log(Rstar) + zdt_sn )

    ZCBLENDH=ZSNWGHT(JL)/(LOG(ZBLENDZ0/ZZ0HSNOW))**2+&
         & (1.0_JPRB-ZSNWGHT(JL))/(LOG(ZBLENDZ0/ZHLOW))**2
    ...
    PZ0MTI(JL,5)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDM))
    PZ0HTI(JL,5)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDH))
    PZ0QTI(JL,5)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDQ))
   ENDIF
