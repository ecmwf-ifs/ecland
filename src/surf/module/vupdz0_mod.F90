...
ASSOCIATE(RCPD=>YDCST%RCPD, RD=>YDCST%RD, RETV=>YDCST%RETV, RG=>YDCST%RG, &
 ...
 & RVZ0ML2D=>PSSDP2(:,SSDP2D_ID%NRVZ0ML2D))
...
!       - URBAN
    IF (LEURBAN) THEN
      ...
      ZMLOW=ZPCVL/(1.0_JPRB-ZPCVH)*RVZ0ML2D(JL)+ZPCVB/(1.0_JPRB-ZPCVH)*RVZ0M(0)
...
DO JL=KIDIA,KFDIA
  ...
  ! Kinematic viscosity of air at 0C: 1.35E-5;
    ...
    IF (LEURBAN) THEN
      ...
      IF (ZURBF.GT.0.0_JPRB) THEN
      ZMLOW=ZPCVL/(1.0_JPRB-ZPCVH)*RVZ0ML2D(JL)+ZPCVB/(1.0_JPRB-ZPCVH)*RVZ0M(0)
      ZHLOW=ZPCVL/(1.0_JPRB-ZPCVH)*RVZ0HL2D(JL)+ZPCVB/(1.0_JPRB-ZPCVH)*RVZ0H(0)
