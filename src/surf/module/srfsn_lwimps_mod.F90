...
! SOLVE IMPLICIT TEMPERATURE EQUATION
    ...
    ZTSTAR=(PTSNM1M(JL)+ZCONS2*(ZHFLUX+ZHFLUXRF1+ZTSURF*ZRS)) &
     & *ZTSTARI
    IF (ZTSTAR <= ZT0) THEN
! NO MELTING OF SNOW
      PTSN(JL)=ZTSTAR
      PGSN(JL)=(ZTSTAR-ZTSURF)*ZRS
    ELSE
! MELTING OF SNOW:
! HEATING PART OF THE TIME STEP
      ZDTHI=1.0_JPRB/(ZHFLUX+ZHFLUXRF1-(ZT0-ZTSURF)*ZRS)
