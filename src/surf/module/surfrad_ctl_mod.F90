...
REAL(KIND=JPRB),    INTENT(INOUT) :: PALBD(KLON,KSW)
REAL(KIND=JPRB),    INTENT(INOUT) :: PALBP(KLON,KSW)
REAL(KIND=JPRB),    INTENT(INOUT) :: PALBTI(KLON,KTILES)
REAL(KIND=JPRB),    INTENT(INOUT) :: PALB(KLON)
REAL(KIND=JPRB),    INTENT(INOUT) :: PSPECTRALEMISS(KLON,KLW) ! dimensioned (KLON,KLW)
!... PEMIT is passed as a pointer rather than an array section in the IFS, so we must keep it assumed shape
REAL(KIND=JPRB),    INTENT(INOUT) :: PEMIT(:)
REAL(KIND=JPRB),    INTENT(INOUT) :: PCCNL(KLON)
REAL(KIND=JPRB),    INTENT(INOUT) :: PCCNO(KLON)
...
REAL(KIND=JPRB) :: ZEMISSTI(KLON,YDRAD%NLWEMISS,KTILES)
...
REAL(KIND=JPRB) :: ZSPECTRALEMISS(KLON,YDRAD%NLWEMISS)
...
!            9 : LAKES                 10 : URBAN
! WATER
  ZEMISSTI(JL,:,1) = REMISS_SEA(1:NLWEMISS)
! SEA-ICE
  ZEMISSTI(JL,:,2) = REMISS_SNOW(1:NLWEMISS)
! WET SKIN
  ZEMISSTI(JL,:,3) = REMISS_LAND(1:NLWEMISS)
! LAKES  
  ZEMISSTI(JL,:,9) = ZEMISSTI(JL,:,1)*ZLWAT(JL) + ZEMISSTI(JL,:,2)*ZLICE(JL)  
...
! DRY SNOW-FREE LOW-VEG
  ...
  ENDIF
  ZEMISSTI(JL,:,4) = REMISS_LAND  (1:NLWEMISS)*ZPROP & 
       &        + REMISS_DESERT(1:NLWEMISS)*(1.0_JPRB-ZPROP)
! SNOW ON LOW-VEG+BARE-SOIL
  ZEMISSTI(JL,:,5) = REMISS_SNOW(1:NLWEMISS)
! DRY SNOW-FREE HIGH-VEG
  ZEMISSTI(JL,:,6) = ZEMISSTI(JL,:,4)
! SNOW UNDER HIGH-VEG
  ZEMISSTI(JL,:,7) = ZEMISSTI(JL,:,4)
! BARE SOIL
  IF (PALBF(JL) > 0.30_JPRB) THEN
!   desert emissivity
    ZEMISSTI(JL,:,8) = REMISS_DESERT(1:NLWEMISS)
  ELSE
    ZEMISSTI(JL,:,8) = ZEMISSTI(JL,:,4)
  ENDIF

! SUM OVER FIRST 8 TILES
  DO I1=1,YDRAD%NLWEMISS
    ZSPECTRALEMISS(JL, I1) = 0
    DO I2=1,8
      ZSPECTRALEMISS(JL, I1) = ZSPECTRALEMISS(JL, I1) + ZEMISSTI(JL, I1, I2) * PFRTI(JL, I2)
    ENDDO
  ENDDO

! URBAN
  IF (LEURBAN) THEN
    ZEMISSTI(JL,:,10) = RURBEMIS
    DO I1=1,YDRAD%NLWEMISS
      ZSPECTRALEMISS(JL, I1) = 0
      DO I2=1,8
        ZSPECTRALEMISS(JL, I1) = ZSPECTRALEMISS(JL, I1) + ZEMISSTI(JL, I1, I2) * PFRTI(JL, I2)
      ENDDO
      ZSPECTRALEMISS(JL, I1) = ZSPECTRALEMISS(JL, I1) + ZEMISSTI(JL, I1, 10) * PFRTI(JL, 10)
    ENDDO
  ENDIF

  IF (LEFLAKE) THEN
    DO I1=1,YDRAD%NLWEMISS
      ZSPECTRALEMISS(JL, I1) = ZSPECTRALEMISS(JL, I1) + PFRTI(JL, 9)*ZEMISSTI(JL, I1, 9)
    ENDDO
  ENDIF
  IF (KLW < NLWEMISS) THEN
    ...
    ! intervals.
    PSPECTRALEMISS(JL,1) = SUM(ZSPECTRALEMISS(JL,:) * REMISS_OLD_WEIGHT(1:NLWEMISS,1))
    PSPECTRALEMISS(JL,2) = SUM(ZSPECTRALEMISS(JL,:) * REMISS_OLD_WEIGHT(1:NLWEMISS,2))
  ELSE
    ! Emissivity has been requested in the same intervals as it was
    ! computed: copy
    PSPECTRALEMISS(JL,1:NLWEMISS) = ZSPECTRALEMISS(JL,:)
  END IF
