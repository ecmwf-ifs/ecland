...
!         should be divided by the sum of fractions 3,4,6 and 8. 
    ...
    ZSURFL5(JL) = ZSSRFL5+ZSLRFL5+ZTHFL5+PGSN5(JL)+PCIL(JL)*PGLICE5(JL)

    ZCLAND=1._JPRB - PFRTI(JL,9)
    IF ( LEFLAKE ) THEN
      IF ( PFRTI(JL,9) > RFRSMALL ) THEN
        ZSURFL5(JL) = PGSN5(JL) + PCIL(JL)*PGLICE5(JL)
        IF ( ZCLAND > RFRSMALL ) THEN
          ZSURFL5(JL) = (PCIL(JL)*PGLICE5(JL)+PGSN5(JL)+ZSSRFL5+ZSLRFL5+ZTHFL5) &
          & / ZCLAND
        ENDIF
      ENDIF
    ENDIF
...
DO JL=KIDIA,KFDIA
  IF (LDLAND(JL)) THEN
    ...
    IF ( LEFLAKE ) THEN
      IF ( PFRTI(JL,9) > RFRSMALL ) THEN
        IF ( ZCLAND > RFRSMALL ) THEN
          PGSN(JL)  = PGSN(JL)+ZSURFL(JL)/ZCLAND
          PGLICE(JL)= PGLICE(JL)+ZSURFL(JL)*PCIL(JL)/ZCLAND
        ...
        PGLICE(JL)=PGLICE(JL)+ZSURFL(JL)*PCIL(JL)
        ZSURFL(JL) = 0.0_JPRB
      ENDIF
    ENDIF
