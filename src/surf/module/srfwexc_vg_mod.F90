...
DO JK=1,KLEVS
  DO JL=KIDIA,KFDIA
    ...
    ELSE
      ZF(JL,JK)=0.0_JPRB
    ENDIF
    ZWPWP3D=PSSDP3(JL,JK,SSDP3D_ID%NRWPWPM3D)
    ZWCAP3D=PSSDP3(JL,JK,SSDP3D_ID%NRWCAPM3D)
    ZLIQ(JL,JK)=MAX(ZWPWP3D,MIN(ZWCAP3D,PWSAM1M(JL,JK)*(1._JPRB-ZF(JL,JK))))
  ENDDO
ENDDO
    ...
    LAYERS: DO JK=1,KLEVS
      DO JL=KIDIA,KFDIA
        ...
        ELSE
          ...
          ZWPWP3D=PSSDP3(JL,JK,SSDP3D_ID%NRWPWPM3D)
          ZROOTW(JL,JK,JTILE)=ZROOTFR*(ZLIQ(JL,JK)-ZWPWP3D+ZEPSILONADD)
        ENDIF
...
!          SUBGRID SATURATION COEFFICIENTS
        ...
        ZWMAX=0.0_JPRB
        DO JK=1,KLEVS
          ZWK=MAX(0.0_JPRB,(PWSAM1M(JL,JK)-PSSDP3(JL,JK,SSDP3D_ID%NRWPWPM3D)))*&
           & (1.0_JPRB-ZF(JL,JK))+PSSDP3(JL,JK,SSDP3D_ID%NRWPWPM3D)
          ZW=ZW+ZFRK(JL,JK)*ZWK*RDAW(JK)                              !Units m
          ZWMK=((ZWSATM-PSSDP3(JL,JK,SSDP3D_ID%NRWPWPM3D))*(1.-ZF(JL,JK))+&
           & PSSDP3(JL,JK,SSDP3D_ID%NRWPWPM3D))
          IF ( LEURBAN ) THEN
           ZWMK=(((1.0_JPRB-PCUR(JL))*ZWSATM + PCUR(JL)*RURBSAT &
             & -PSSDP3(JL,JK,SSDP3D_ID%NRWPWPM3D))*(1.-ZF(JL,JK))+&
             & PSSDP3(JL,JK,SSDP3D_ID%NRWPWPM3D))
          ENDIF
