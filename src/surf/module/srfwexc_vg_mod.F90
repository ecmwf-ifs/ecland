...
REAL(KIND=JPRB) :: Z_RHOH20, ZD, &
 ...
 & ZCONW1, ZLYEPS, ZLYSIC, ZVOL, ZROS, ZSUM, ZLIMRS, ZWSATM, ZWRESTM
...
ASSOCIATE(LESSRO=>YDSOIL%LESSRO, RDAW=>YDSOIL%RDAW, RDMAXM=>YDSOIL%RDMAXM, &
 & RDMINM=>YDSOIL%RDMINM, RLAMBDAM3D=>PSSDP3(:,:,SSDP3D_ID%NRLAMBDAM3D), RMFACM=>YDSOIL%RMFACM, &
 ...
 & RWCAPM=>YDSOIL%RWCAPM, RWCONSM3D=>PSSDP3(:,:,SSDP3D_ID%NRWCONSM3D), RWPWPM=>YDSOIL%RWPWPM, &
 & RWRESTM3D=>PSSDP3(:,:,SSDP3D_ID%NRWRESTM3D), RWSATM3D=>PSSDP3(:,:,SSDP3D_ID%NRWSATM3D), &
 & RVROOTSA=>YDVEG%RVROOTSA, LEURBAN=>YDURB%LEURBAN, RURBALP=>YDURB%RURBALP, RURBCON=>YDURB%RURBCON,&
...
!          SUBGRID SATURATION COEFFICIENTS
        ...
        ZWMK(:)=((ZWSATM-RWPWPM(JS))*(1.-ZF(JL,:))+RWPWPM(JS))
        IF ( LEURBAN ) THEN
