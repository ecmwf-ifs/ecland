...
!           ZGMEST(JL)=EXP(RASW-RBW*ZFZEROT(JL))
     ...
     IF (KVTYPE(JL)>=0._JPRB) THEN
       ZAMMAX(JL)=(ZVAMMAX*EXP(Z1(JL)*LOG(ZVQDAMMAX))) &
          & /((1._JPRB+EXP(0.3_JPRB*(ZVT1AMMAX-ZTSK(JL))))* &
          & (1._JPRB+EXP(0.3_JPRB*(ZTSK(JL)-ZVT2AMMAX))))
       ZGMEST(JL)=(ZGMEST(JL)*EXP(Z1(JL)*LOG(ZVQDGMES))) &
          & /((1._JPRB+EXP(0.3_JPRB*(ZVT1GMES-ZTSK(JL))))* &
          & (1._JPRB+EXP(0.3_JPRB*(ZTSK(JL)-ZVT2GMES))))
     ENDIF
    ENDIF !LDLAND
