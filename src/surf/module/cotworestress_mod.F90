...
! initialization of local variables
 ...
 ! Skin temperature : convert from K to C
  ...
  ! Add soil moisture stress effect to leaf conductance:
     ...
     IF (KTILE==4) THEN
        ZDMAXT(JL) = PSSDP2(JL,SSDP2D_ID%NRVDMAXL2D)
        ZRVEPSO(JL)= PSSDP2(JL,SSDP2D_ID%NRVEPSOL2D)
        ZVF2I(JL)  = PSSDP2(JL,SSDP2D_ID%NRVF2IL2D)
        ZFZEROT(JL)= PSSDP2(JL,SSDP2D_ID%NRVFZEROSTL2D)
        ZRVGC(JL)  = PSSDP2(JL,SSDP2D_ID%NRVGCL2D)
        ZGMEST(JL) = PSSDP2(JL,SSDP2D_ID%NRVGMESL2D)
        ZVAMMAX    = PSSDP2(JL,SSDP2D_ID%NRVAMMAXL2D)
        ZVGAMM     = PSSDP2(JL,SSDP2D_ID%NRVGAMML2D)
        ZVQDAMMAX  = PSSDP2(JL,SSDP2D_ID%NRVQDAMMAXL2D)
        ZVQDGAMM   = PSSDP2(JL,SSDP2D_ID%NRVQDGAMML2D)
        ZVQDGMES   = PSSDP2(JL,SSDP2D_ID%NRVQDGMESL2D)
        ZVT1AMMAX  = PSSDP2(JL,SSDP2D_ID%NRVT1AMMAXL2D)
        ZVT2AMMAX  = PSSDP2(JL,SSDP2D_ID%NRVT2AMMAXL2D)
        ZVT1GMES   = PSSDP2(JL,SSDP2D_ID%NRVT1GMESL2D)
        ZVT2GMES   = PSSDP2(JL,SSDP2D_ID%NRVT2GMESL2D)
!       ZVAH  = RVAHL2D(JL)
!       ZVBH  = RVBHL2D(JL) 
     ELSE IF (KTILE==6 .OR. KTILE==7 ) THEN
        ZDMAXT(JL) = PSSDP2(JL,SSDP2D_ID%NRVDMAXH2D)
        ZRVEPSO(JL)= PSSDP2(JL,SSDP2D_ID%NRVEPSOH2D)
        ZVF2I(JL)  = PSSDP2(JL,SSDP2D_ID%NRVF2IH2D)
        ZFZEROT(JL)= PSSDP2(JL,SSDP2D_ID%NRVFZEROSTH2D)
        ZRVGC(JL)  = PSSDP2(JL,SSDP2D_ID%NRVGCH2D)
        ZGMEST(JL) = PSSDP2(JL,SSDP2D_ID%NRVGMESH2D)
        ZVAMMAX    = PSSDP2(JL,SSDP2D_ID%NRVAMMAXH2D)
        ZVGAMM     = PSSDP2(JL,SSDP2D_ID%NRVGAMMH2D)
        ZVQDAMMAX  = PSSDP2(JL,SSDP2D_ID%NRVQDAMMAXH2D)
        ZVQDGAMM   = PSSDP2(JL,SSDP2D_ID%NRVQDGAMMH2D)
        ZVQDGMES   = PSSDP2(JL,SSDP2D_ID%NRVQDGMESH2D)
        ZVT1AMMAX  = PSSDP2(JL,SSDP2D_ID%NRVT1AMMAXH2D)
        ZVT2AMMAX  = PSSDP2(JL,SSDP2D_ID%NRVT2AMMAXH2D)
        ZVT1GMES   = PSSDP2(JL,SSDP2D_ID%NRVT1GMESH2D)
        ZVT2GMES   = PSSDP2(JL,SSDP2D_ID%NRVT2GMESH2D)
!       ZVAH  = 5.323_JPRB
!       ZVBH  = -0.8929_JPRB
     ENDIF
     IF (KVTYPE(JL) == 1 .OR. KVTYPE(JL) == 2 .OR. KVTYPE(JL) == 7 .OR. KVTYPE(JL) == 9 .OR. &
       ...
       ZDMAXSTAR(JL)=EXP((LOG(ZGMEST(JL)*1000._JPRB)-PSSDP2(JL,SSDP2D_ID%NRVAHL2D))/PSSDP2(JL,SSDP2D_ID%NRVBHL2D))/1000._JPRB
    ! compensation point: temperature response
       ...
       IF (LVSTRESS(KVTYPE(JL)) .AND. (PF2(JL) >= ZVF2I(JL))) THEN
           ZDMAXT(JL)=RDMAXN+(ZDMAXSTAR(JL)-RDMAXN)*(PF2(JL)-ZVF2I(JL))/(1._JPRB-ZVF2I(JL))
            ZGMEST(JL)=EXP(PSSDP2(JL,SSDP2D_ID%NRVAHL2D)+PSSDP2(JL,SSDP2D_ID%NRVBHL2D)*LOG(ZDMAXT(JL)*1000._JPRB))/1000._JPRB
       ...
       ELSEIF (LVSTRESS(KVTYPE(JL)) .AND. (PF2(JL) < ZVF2I(JL))) THEN
           ZDMAXT(JL)=RDMAXN
           ZGMEST(JL)=(EXP(PSSDP2(JL,SSDP2D_ID%NRVAHL2D)+PSSDP2(JL,SSDP2D_ID%NRVBHL2D)*LOG(RDMAXN*1000._JPRB))/1000._JPRB)*&
                & PF2(JL)/ZVF2I(JL)
       ...
       ELSEIF (.NOT. LVSTRESS(KVTYPE(JL)) .AND. (PF2(JL) < ZVF2I(JL))) THEN
           ...
           ZGMEST(JL)=EXP(PSSDP2(JL,SSDP2D_ID%NRVAHL2D)+PSSDP2(JL,SSDP2D_ID%NRVBHL2D)*LOG(RDMAXX*1000._JPRB))/1000._JPRB
       ENDIF
       ...
       IF (.NOT. LVSTRESS(KVTYPE(JL)) .AND. ((ZDMAXT(JL) <= RDMAXN)) ) THEN
            ...
            ZDMAXT(JL) = RDMAXN
            ZGMEST(JL) = (EXP(PSSDP2(JL,SSDP2D_ID%NRVAHL2D)+PSSDP2(JL,SSDP2D_ID%NRVBHL2D)*LOG(RDMAXN*1000._JPRB))/1000._JPRB)*PF2(JL)/ZVF2I(JL)
       ENDIF
