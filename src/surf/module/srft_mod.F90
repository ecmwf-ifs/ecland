...
REAL(KIND=JPRB) :: ZCONS1, ZCONS2, ZSLRFL, ZSSRFL, ZTHFL, ZTMST,&
 & ZFF, ZWU, ZLSM, ZLIC, ZLWT, ZLAMBDASAT, ZKERSTEN, ZINVWSAT, ZWSATM, &
 & ZLAMBDADRY3D, ZLAMSAT13D
...
ASSOCIATE(RLVTT=>YDCST%RLVTT, &
 ...
 & RLAMBDADRY=>YDSOIL%RLAMBDADRY, RLAMBDAICE=>YDSOIL%RLAMBDAICE, RLAMBDAWAT=>YDSOIL%RLAMBDAWAT, &
 & RLAMSAT1=>YDSOIL%RLAMSAT1, RRCSOIL=>YDSOIL%RRCSOIL, RSIMP=>YDSOIL%RSIMP, RWSAT=>YDSOIL%RWSAT, &
 & RURBTC1=>YDURB%RURBTC1,RURBVHC=>YDURB%RURBVHC)
...
! added fix to be consistent with Peters-Lidard et al. 1998 
     ...
     ! ZLWT=RLAMBDAWAT**ZWU
      IF(LEVGEN)THEN
        ZLAMBDADRY3D=PSSDP3(JL,JK,SSDP3D_ID%NRLAMBDADRYM3D)
        ZLAMSAT13D=PSSDP3(JL,JK,SSDP3D_ID%NRLAMSAT1M3D)
        ZLSM=ZLAMSAT13D
        ZWSATM=PSSDP3(JL,JK,SSDP3D_ID%NRWSATM3D)

        ZWU=ZWSATM*(1.0_JPRB-ZFF)
        ZLWT=RLAMBDAWAT**ZWU

        ZINVWSAT=1.0_JPRB/ZWSATM
        ZLIC=RLAMBDAICE**(ZWSATM-ZWU)

        ZLAMBDASAT=ZLSM*ZLIC*ZLWT

        ZKERSTEN=RKERST2*LOG10(MAX(RKERST1,PWSAM1M(JL,JK)*ZINVWSAT))+RKERST3
        ZDIF(JL,JK)=ZLAMBDADRY3D+ZKERSTEN*(ZLAMBDASAT-ZLAMBDADRY3D)
        IF (YDSOIL%LESOILCOND ) THEN
          ZDIF(JL,JK)=FSOILTCOND(PWSAM1M(JL,JK),ZLAMBDADRY3D,ZWSATM,ZLAMSAT13D, ZFF)
