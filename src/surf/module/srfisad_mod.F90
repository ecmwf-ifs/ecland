...
SUBROUTINE SRFISAD(KIDIA , KFDIA , KLON  , KLEVS ,&
 ...
 & PSLRFL5 , PSSRFLTI5 , PTIA5   , LDICE   , LDNH,LESNICE,&
 & YDCST   , YDSOIL    ,&
  ...
  DO JL=KIDIA,KFDIA
    IF (LDICE(JL)) THEN
      ...
      IF (LESNICE) THEN
        ...
        ZSURFL5(JL)=(PGSN5(JL) + &
                    PFRTI(JL,2)*(PSSRFLTI5(JL,2)+PSLRFL5(JL)) + & 
                    ZTHFL5)/MAX(ZEPSILON,(PFRTI(JL,2)+PFRTI(JL,5)))
      ENDIF
    ENDIF
  ENDDO
  ...
  DO JL=KIDIA,KFDIA
    IF (LDICE(JL)) THEN
      ...
      ELSE
        ...
        PSSRFLTI(JL,2)= PSSRFLTI(JL,2)+ZSURFL(JL)*PFRTI(JL,2)/MAX(ZEPSILON,(PFRTI(JL,2)+PFRTI(JL,5)))
        PSLRFL(JL)    = PSLRFL(JL)+ZSURFL(JL)*PFRTI(JL,2)/MAX(ZEPSILON,(PFRTI(JL,2)+PFRTI(JL,5)))
        ZTHFL         = ZTHFL+ZSURFL(JL)/MAX(ZEPSILON,(PFRTI(JL,2)+PFRTI(JL,5)))
        ZSURFL(JL)    = 0._JPRB
        PAHFSTI (JL,2)= PAHFSTI(JL,2)+ZTHFL*PFRTI(JL,2)
        PEVAPTI (JL,2)= PEVAPTI(JL,2)+ZTHFL*RLSTT*PFRTI(JL,2)
        ZTHFL=0._JPRB
      ENDIF
    ENDIF
  ENDDO
