...
! so different definition of heights etc.
  ...
  ZL_S=ZNLEV_S/ZDL_S
  IF (ZDL_S  >  RCHBHDL) THEN
    ZNLEV_S=RCHBHDL*ZL_S+MAX(PZ0MSM(JL),PZ0HM(JL))
    ZDL_S=ZNLEV_S*ZDL_S/(PGEOMLEV(JL)/RG+PZ0MSM(JL))
  ENDIF

  Z2MP =Z2M+PZ0MSM(JL)
  IF (Z2MP/ZL_S  >  RCHBHDL) THEN
    Z2MP=RCHBHDL*ZL_S+MAX(PZ0MSM(JL),PZ0HM(JL))
  ENDIF
