...
!    G. Balsamo 27/08/2021 - postprocessing based on tile average for T2m RH2m
...
REAL(KIND=JPRB) :: ZFRMAX(KLON)   , ZFRLMAX(KLON)  , ZALB(KLON)     , &
                 ...
                 & ZCBLENDM(KLON) , ZCBLENDH(KLON) , ZCBLENDQ(KLON) , &
                 & ZSL(KLON)      , &
...
ASSOCIATE(LEOCWA=>YDEXC%LEOCWA, LEOCCO=>YDEXC%LEOCCO, REPDU2=>YDEXC%REPDU2, &
 ...
 & LEFLAKE=>YDFLAKE%LEFLAKE,LEURBAN=>YDURBAN%LEURBAN,RH_ICE_MIN_FLK=>YDFLAKE%RH_ICE_MIN_FLK, &
...
!               not needed for tile-average postprocessing
!FRMAX(KIDIA:KFDIA)=PFRTI(KIDIA:KFDIA,1)
!FRLMAX(KIDIA:KFDIA)=PFRTI(KIDIA:KFDIA,1)
!FRMAX(KIDIA:KFDIA)=1
!FRLMAX(KIDIA:KFDIA)=1
!O JTILE=2,KTILES
! DO JL=KIDIA,KFDIA
...
!*         Use tile average (log) Z0 for M and H and Q
...
ZCBLENDQ(KIDIA:KFDIA)=PFRTI(KIDIA:KFDIA,1)&
           &/(LOG(ZBLENDZ0/ZZ0QTI(KIDIA:KFDIA,1)))**2
DO JTILE=2,KTILES
  DO JL=KIDIA,KFDIA
    ...
    ZCBLENDQ(JL)=ZCBLENDQ(JL)&
           &+PFRTI(JL,JTILE)/(LOG(ZBLENDZ0/ZZ0QTI(JL,JTILE)))**2
  ENDDO
ENDDO
...
DO JL=KIDIA,KFDIA
  ...
  PZ0QW(JL)=ZBLENDZ0*EXP(-1._JPRB/SQRT(ZCBLENDQ(JL)))
  PZ0HW(JL)=PZ0H(JL)
ENDDO
...
ELSE
  IITT=1
ENDIF
PKHLEV(KIDIA:KFDIA)=0.0_JPRB
PKCLEV(KIDIA:KFDIA)=0.0_JPRB
...
!      PKCLEV(JL)=ZKCLEV(JL)
!    ENDIF
    PKHLEV(JL)=PKHLEV(JL)+PFRTI(JL,JTILE)*ZKHLEV(JL)
    PKCLEV(JL)=PKCLEV(JL)+PFRTI(JL,JTILE)*ZKCLEV(JL)
  ENDDO
ENDDO
...
!          ROUGHNESS LENGTH FOR HEAT and MOISTURE ARE CALCULATE
!          AS WEIGHTED AVERAGE OF THE TILES VALUES
DO JL=KIDIA,KFDIA
  PCPTSPP(JL)=0.0_JPRB
  PQSAPP(JL)=0.0_JPRB
  PBUOMPP(JL)=0.0_JPRB
  PZDLPP(JL)=0.0_JPRB
  DO JTILE=1,KTILES
    PCPTSPP(JL)=PCPTSPP(JL)+PCPTSTI(JL,JTILE)*PFRTI(JL,JTILE)
     PQSAPP(JL)= PQSAPP(JL)+ZZQSATI(JL,JTILE)*PFRTI(JL,JTILE)
    PBUOMPP(JL)=PBUOMPP(JL)+ZBUOMTI(JL,JTILE)*PFRTI(JL,JTILE)
     PZDLPP(JL)= PZDLPP(JL)+ ZZDLTI(JL,JTILE)*PFRTI(JL,JTILE)
  ENDDO
ENDDO
