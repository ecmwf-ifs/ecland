...
!*    ENDIF
  ...
  CASE(4)
    DO JL=KIDIA,KFDIA
      IF (PTSKM1M(JL,JT) > PTSRF(JL,JT)) THEN
        ZLAMSK(JL,JT)=PSSDP2(JL,SSDP2D_ID%NRVLAMSKSL2D)
      ELSE
        ZLAMSK(JL,JT)=PSSDP2(JL,SSDP2D_ID%NRVLAMSKL2D)
      ENDIF
    ENDDO
  ZFRSR=1.0_JPRB-RVTRSR(1)
  CASE(5)
  ...
  CASE(6)
     DO JL=KIDIA,KFDIA
      IF (PTSKM1M(JL,JT) > PTSRF(JL,JT)) THEN
        ZLAMSK(JL,JT)=PSSDP2(JL,SSDP2D_ID%NRVLAMSKSH2D)
      ELSE
        ZLAMSK(JL,JT)=PSSDP2(JL,SSDP2D_ID%NRVLAMSKH2D)
      ENDIF
    ENDDO
  ...
  CASE(7)
     DO JL=KIDIA,KFDIA
      IF (PTSKM1M(JL,JT) > PTSRF(JL,JT)) THEN
        ZLAMSK(JL,JT)=PSSDP2(JL,SSDP2D_ID%NRVLAMSKSH2D)/(1._JPRB+ZSTABEXSN(JL)*PSSDP2(JL,SSDP2D_ID%NRVLAMSKSH2D))
      ELSE
