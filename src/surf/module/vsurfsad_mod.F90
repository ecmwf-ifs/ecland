...
REAL(KIND=JPRB) :: ZF2LP15(KLON), ZF2LP25(KLON), ZF2L5(KLON)
REAL(KIND=JPRB) :: ZF2HP15(KLON), ZF2HP25(KLON), ZF2H5(KLON)
REAL(KIND=JPRB) :: ZF2BP15(KLON), ZF2BP25(KLON), ZF2B5(KLON)
...
REAL(KIND=JPRB) ::   &
 & ZCOR, ZEPSF3, ZF, ZF1H, ZF1L, ZF2H, ZF2L, ZF2B, ZF3H, ZF3L,   &
...
!           soil moisture stress function : F2
    ...
    IF (PTSAM1M5(JL,1) <= RTT ) THEN
      ...
      ZF2H5(JL)=RCEPSW
    ELSE
      ZF2LP15(JL) = (ZWROOTL5(JL)-ZWPWP)*ZQWEVAP
      IF (ZF2LP15(JL) < 1.0_JPRB) THEN
        ZF2LP25(JL) = ZF2LP15(JL)
      ELSE
        ZF2LP25(JL) = 1.0_JPRB
      ENDIF
      IF (ZF2LP25(JL) > RCEPSW) THEN
        ZF2L5(JL) = ZF2LP25(JL)
      ELSE
        ZF2L5(JL) = RCEPSW
      ENDIF
  
      ZF2HP15(JL) = (ZWROOTH5(JL)-ZWPWP)*ZQWEVAP
      IF (ZF2HP15(JL) < 1.0_JPRB) THEN
        ZF2HP25(JL) = ZF2HP15(JL)
      ELSE
        ZF2HP25(JL) = 1.0_JPRB
      ENDIF
      IF (ZF2HP25(JL) > RCEPSW) THEN
        ZF2H5(JL) = ZF2HP25(JL)
      ELSE
        ZF2H5(JL) = RCEPSW
      ENDIF
    ...
    ZF2BP15(JL) = (ZLIQ5(JL,1)-ZWPWP)*ZQWEVAP
    IF (ZF2BP15(JL) < 1.0_JPRB) THEN
      ZF2BP25(JL) = ZF2BP15(JL)
    ELSE
      ZF2BP25(JL) = 1.0_JPRB
    ENDIF
    IF (ZF2BP25(JL) > RCEPSW) THEN
      ZF2B5(JL) = ZF2BP25(JL)
    ELSE
      ZF2B5(JL) = RCEPSW
    ENDIF
...
!           soil moisture stress function : F2
    ...
    ENDIF
    IF (ZF2BP15(JL) < 1.0_JPRB) THEN
      ZF2BP1 = ZF2BP1+ZF2BP2
    ENDIF
    ZLIQ(JL,1) = ZLIQ(JL,1)+ZQWEVAP*ZF2BP1
...
!   if first soil layer temperature is freezing then shutdown transpiration
      ...
      ENDIF
      IF (ZF2HP15(JL) < 1.0_JPRB) THEN
        ZF2HP1 = ZF2HP1+ZF2HP2
      ENDIF
      ZWROOTH = ZWROOTH+ZQWEVAP*ZF2HP1
      IF (ZF2LP25(JL) > RCEPSW) THEN
        ZF2LP2 = ZF2LP2+ZF2L
      ENDIF
      IF (ZF2LP15(JL) < 1.0_JPRB) THEN
        ZF2LP1 = ZF2LP1+ZF2LP2
      ENDIF
    ENDIF

    ZWROOTL = ZWROOTL+ZQWEVAP*ZF2LP1
