...
REAL(KIND=JPRB) :: ZSNM1M(KLON), ZRSNM1M(KLON)
LOGICAL :: LDSNOWOVERLAND, LDSNOWOVERICE
...
!*  IF ( LESNICE ) THEN
!*    LDSNOWOVERLAND=.FALSE.
!*    LDSNOWOVERICE=.TRUE.
!*    ZSNM1M(:)=SUM(PSNM1M(:,1:KLEVSN),DIM=2)
!*    ZRSNM1M(:)=SUM(PSNM1M(:,1:KLEVSN),DIM=2)/SUM( (PSNM1M(:,1:KLEVSN)/PRSNM1M(:,1:KLEVSN)),DIM=2 )
!*    CALL SRFSN_LWIMP(KIDIA  ,KFDIA  ,KLON   ,LDLAND, LDSNOWOVERLAND, PTSPHY, &
!*    & ZSNM1M ,PTSNM1M(:,1) ,PASNM1M, ZRSNM1M,PTSAM1M,PTIAM1M,PHLICEM1M,  &
...
ELSE
  ...
  !* LIQUID WATER DIAGNOSTIC
    LDSNOWOVERLAND=.TRUE.
    LDSNOWOVERICE=.TRUE.
    CALL SRFSN_LWIMP(KIDIA  ,KFDIA  ,KLON   ,KTILES   ,LDLAND, LDSNOWOVERLAND, PTSPHY, &
...
IF (LEVGEN) THEN
 ...
 & PSSDP3,PTSPHY,KTVL,KTVH,KSOTY,PSDOR,PFRTI,PEVAPTI,&
