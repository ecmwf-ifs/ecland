...
REAL(KIND=JPRB) :: ZLAMBDADRY3D, ZLAMSAT13D, ZWSATM3D
...
! add fix to be consistent with Peters-Lidard et al. 1998 
      ...
      IF (PCIL(JL)<1._JPRB-10._JPRB*ZEPSILON .AND. LDLAND(JL))THEN
        ZLAMBDADRY3D=PSSDP3(JL,1,SSDP3D_ID%NRLAMBDADRYM3D)
        ZWSATM3D=PSSDP3(JL,1,SSDP3D_ID%NRWSATM3D)
        ZLAMSAT13D=PSSDP3(JL,1,SSDP3D_ID%NRLAMSAT1M3D)
        ZSURFCOND(JL) = MAX(0.19_JPRB,MIN(2._JPRB,FSOILTCOND(PWSAM1M(JL,1),ZLAMBDADRY3D,&
               & ZWSATM3D,ZLAMSAT13D,ZFF)))
