...
SUBROUTINE SURFSEBS_CTL(KIDIA,KFDIA,KLON,KTILES,LDSICE,KTVL,KTVH,&
 & PTMST,LREPEAT,PSSKM1M,PTSKM1M,PQSKM1M,PDQSDT,PRHOCHU,PRHOCQU,&
...
DO JL=KIDIA,KFDIA
  ZSTABEXSN(JL)=PTMST/(MAX(RQSNCR,(PSNS(JL,1)/PRSN(JL,1)))*RHOCI*PRSN(JL,1)/RHOICE)
ENDDO 
...
END SUBROUTINE SURFSEBS_CTL
END MODULE SURFSEBS_CTL_MOD
