...
!   - Ocean open water
IF( LWCOU2W .AND. LWCOUHMF) THEN
  ! With direct sea state effect on heat and moisture:
  DO JL=KIDIA,KFDIA
    Z0Q = RNUQ*ZDIV65(JL)
...
ELSE
  DO JL=KIDIA,KFDIA
    ZUST(JL,JTILE) = ZUST(JL,JTILE) &
     & - RNUQ*ZDIV65(JL)*ZDIV65(JL)*PZ0QTI(JL,JTILE)
    PZ0QTI(JL,JTILE) = 0.0_JPRB
    ZUST(JL,JTILE) = ZUST(JL,JTILE) &
     & - RNUH*ZDIV65(JL)*ZDIV65(JL)*PZ0HTI(JL,JTILE)
    PZ0HTI(JL,JTILE) = 0.0_JPRB
    ZUST(JL,JTILE) = ZUST(JL,JTILE) &
     & - RNUM*ZDIV65(JL)*ZDIV65(JL)*PZ0MTI(JL,JTILE)
    ZUST2(JL,JTILE) = ZUST2(JL,JTILE)+(ZRGI*PCHAR(JL))*PZ0MTI(JL,JTILE)
