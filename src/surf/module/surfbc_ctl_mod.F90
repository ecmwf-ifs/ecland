...
SUBROUTINE SURFBC_CTL(KIDIA, KFDIA, KLON, KTILES,KLEVSN, &
 ...
 & PCVL, PCVH, PCUR, PCIL, PLAIL, PLAIH, PLAIHP, &
 & PWLMX, PFRTI, &
 & PCSN, PSSDP2, &
...
REAL(KIND=JPRB),    INTENT(OUT) :: PCIL(:)
...
DO JL=KIDIA,KFDIA
  ...
  LDSICE(JL)=(PCI(JL) > RCIMIN).AND.(.NOT. LDLAND(JL)).AND.(.NOT. LDLAKE(JL))  ! FOR SEAMLESS TREATMENT OF ICE OVER LAND AND WATER
  LDLICE(JL)=((PCI(JL) > RCIMIN).AND.(LDLAND(JL)))  ! FOR SEAMLESS TREATMENT OF ICE OVER LAND
...
DO JL=KIDIA,KFDIA
  ...
  ELSE
    ...
    ENDIF
      IF (ZPCVLK+PCI(JL)>1.0_JPRB)THEN
        ZFRTIT=1._JPRB-ZPCVLK
      ENDIF      
      PCIL(JL)=ZFRTIT
      PFRTI(JL,2)=ZFRTIT*(1.0_JPRB-ZCVS(JL))
      ZPCVL=PCVL(JL)*(1.0_JPRB-PCLAKE(JL)-ZFRTIT)    ! LOW VEGETATION FRACTION
      ZPCVH=PCVH(JL)*(1.0_JPRB-PCLAKE(JL)-ZFRTIT)    ! HIGH VEGETATION FRACTION
      ZPCVB=1.0_JPRB-ZPCVL-ZPCVH-PCLAKE(JL) -ZFRTIT  ! BARE SOIL FRACTION
