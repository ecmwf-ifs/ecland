...
ASSOCIATE(RCA_1X_CO2_NIT=>YDAGS%RCA_1X_CO2_NIT, &
 ...
 & RVBSLAI_NITRO=>YDAGS%RVBSLAI_NITRO, &
...
IF (PCV(JL) > 0._JPRB) THEN
  ...
  ZSEFOLD=ZVSEFOLD*MIN(1.0_JPRB,PANFM(JL)/RVANMAX(IVT))*   &
   & MAX(((ZVGMES*1000._JPRB)**0.321_JPRB)*PLAI(JL)/ZLAIB_NITRO,1._JPRB)/RDAY
...
! senesence of active biomass
  ZXM=MIN(PBIOMASS(JL)-ZLAIMIN*RVBSLAI_NITRO(IVT), &
