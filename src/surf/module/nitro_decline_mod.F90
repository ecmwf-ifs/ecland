...
CONTAINS
SUBROUTINE NITRO_DECLINE(KIDIA,KFDIA,JVT,KVT,KLON,KSTEP, PTSPHY,&
 & PCV,&
...
INTEGER(KIND=JPIM),INTENT(IN)    :: JVT
...
REAL(KIND=JPRB) :: ZVGMES, ZLAIMIN, ZVSEFOLD
REAL(KIND=JPRB) :: ZVBSLAI_NITRO, ZVANMAX
...
ASSOCIATE(RCA_1X_CO2_NIT=>YDAGS%RCA_1X_CO2_NIT, &
 ...
 & RTAULIM=>YDAGS%RTAULIM, &
 & RVANMAXL2D=>PSSDP2(:,SSDP2D_ID%NRVANMAXL2D), RVANMAXH2D=>PSSDP2(:,SSDP2D_ID%NRVANMAXH2D), &
 ...
 & RVSEFOLDL2D=>PSSDP2(:,SSDP2D_ID%NRVSEFOLDL2D), RVSEFOLDH2D=>PSSDP2(:,SSDP2D_ID%NRVSEFOLDH2D), &
 & RDAY=>YDCST%RDAY, RPI=>YDCST%RPI, RTT=>YDCST%RTT, &
 & RCC_NIT_BASE_DIL=>YDAGS%RCC_NIT_BASE_DIL, RESPBSTR_COEF=>YDAGS%RESPBSTR_COEF, &
 & RLAIB_NITROA=>YDAGS%RLAIB_NITROA, RLAIB_NITROB=>YDAGS%RLAIB_NITROB, &
 & RLAIB_NITROMIN=>YDAGS%RLAIB_NITROMIN, RVGMES_FLDEXP=>YDAGS%RVGMES_FLDEXP)
...
! coef c for biomass in kg/m2 
  ZCC_NITRO=RCC_NIT/(RCC_NIT_BASE_DIL**RCA_1x_CO2_NIT)
...
DO JL=KIDIA,KFDIA
   ...
   IF (JVT .EQ. 1_JPIM) THEN
      ...
      ZVSEFOLD = PSSDP2(JL,SSDP2D_ID%NRVSEFOLDL2D)
      ZVGMES   = PSSDP2(JL,SSDP2D_ID%NRVGMESL2D)
      ZVBSLAI_NITRO = PSSDP2(JL,SSDP2D_ID%NRVBSLAI_NITROL2D)
      ZVANMAX  = PSSDP2(JL,SSDP2D_ID%NRVANMAXL2D)
   ELSE IF (JVT .EQ. 2_JPIM) THEN
      ! low vegetation
      ZLAIMIN  = PSSDP2(JL,SSDP2D_ID%NRVLAIMINH2D)
      ZVSEFOLD = PSSDP2(JL,SSDP2D_ID%NRVSEFOLDH2D)
      ZVGMES   = PSSDP2(JL,SSDP2D_ID%NRVGMESH2D)
      ZVBSLAI_NITRO = PSSDP2(JL,SSDP2D_ID%NRVBSLAI_NITROH2D)
      ZVANMAX  = PSSDP2(JL,SSDP2D_ID%NRVANMAXH2D)
   ELSE
...
IF (PCV(JL) > 0._JPRB) THEN
  ...
  ZLAIB_NITRO=MAX(RLAIB_NITROA-RLAIB_NITROB*ATAN(ABS(PLAT(JL))*RPI/180._JPRB),RLAIB_NITROMIN)

  ZSEFOLD=ZVSEFOLD*MIN(1.0_JPRB,PANFM(JL)/ZVANMAX)*   &
   & MAX(((ZVGMES*1000._JPRB)**RVGMES_FLDEXP)*PLAI(JL)/ZLAIB_NITRO,1._JPRB)/RDAY
...
! (the user-defined RTAULIM) of the maximum e-folding time.  
  ZSEFOLD=MAX((RTAULIM/100._JPRB)*ZVSEFOLD/(RDAY),ZSEFOLD)
  
! senesence of active biomass
  ZXM=MIN(PBIOMASS(JL)-ZLAIMIN*ZVBSLAI_NITRO, &
...
! senesence of deep-structural biomass
  ZMORTBSTR2=ZBIOMASSTR2*(1.0_JPRB-EXP(-1.0_JPRB*RDAY/ZVSEFOLD))
!1
!ENDIF
...
!   above-ground structure biomass increment and storage
    ...
    ZMORTBSTR=ZBIOMASSTR*(1.0_JPRB-EXP(-1.0_JPRB*RDAY/ZVSEFOLD))
!2
...
!   the structural biomass dies exponetially at the lowest rate    
    ZSTORAGE=0.0_JPRB
    ZMORTBSTR=PBIOMASSTR_LAST(JL)*(1.0_JPRB-EXP(-1.0_JPRB*RDAY/ZVSEFOLD))
!3
...
ZTENTHLOG2=LOG(RESPBSTR_COEF)*0.1_JPRB
DO JL=KIDIA,KFDIA
 IF (PCV(JL) > 0._JPRB) THEN
