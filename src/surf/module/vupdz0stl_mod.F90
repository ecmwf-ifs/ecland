...
!       - Wet skin
    ...
    PZ0MTI5(JL,3) = PCVL(JL)*RVZ0M(KTVL(JL))+PCVH(JL)*RVZ0M(KTVH(JL))&
     & +(1.-PCVL(JL)-PCVH(JL))*RVZ0M(0)  
!       - Low Vegetation
    PZ0MTI (JL,4) = 0.0_JPRB
    PZ0MTI5(JL,4) = RVZ0M(KTVL(JL))
!       - Exposed snow
    PZ0MTI (JL,5) = 0.0_JPRB
    PZ0MTI5(JL,5) = RVZ0M(12)
!       - High vegetation
    PZ0MTI (JL,6) = 0.0_JPRB
    PZ0MTI5(JL,6) = RVZ0M(KTVH(JL))
!       - Sheltered snow
    PZ0MTI (JL,7) = 0.0_JPRB
    PZ0MTI5(JL,7) = RVZ0M(KTVH(JL))
!       - Bare soil
    PZ0MTI (JL,8) = 0.0_JPRB
    PZ0MTI5(JL,8) = RVZ0M(0)
    IF (LEFLAKE) THEN
!       - Preliminary value for lake
    ...
    IF (LEURBAN) THEN
      ...
      IF (ZURBF.GT.0.0_JPRB) THEN
       PZ0MTI5(JL,3)=ZPCVL*RVZ0M(KTVL(JL))+ZPCVH*RVZ0M(KTVH(JL))&
         & +ZPCVB*RVZ0M(0)+ZURBF*RURBZTM
       PZ0MTI5(JL,5)=(ZPCVL/(1.0_JPRB-ZPCVH))*RVZ0M(12)&
...
!     APPLY W* CORRECTION
    ...
    IF (PBUOMTI5(JL,JTILE) > 0.0_JPRB) THEN
      ...
      ZUST25(JL,JTILE) = ZUST25(JL,JTILE)+ZCDFC*ZWST25
    ENDIF
...
DO JL=KIDIA,KFDIA
  PZ0MTI (JL,JTILE) = 0.0_JPRB
  PZ0MTI5(JL,JTILE) = RVZ0M(12)
  PZ0HTI (JL,JTILE) = 0.0_JPRB
  PZ0HTI5(JL,JTILE) = RVZ0H(12)
  PZ0QTI (JL,JTILE) = PZ0HTI (JL,JTILE)
  PZ0QTI5(JL,JTILE) = PZ0HTI5(JL,JTILE)
ENDDO
