...
ASSOCIATE(RCPD=>YDCST%RCPD, RETV=>YDCST%RETV, RLSTT=>YDCST%RLSTT, &
 ...
 & RWPWPM=>YDSOIL%RWPWPM, RWRESTM3D=>PSSDP3(:,:,SSDP3D_ID%NRWRESTM3D), &
  ...
  DO JK=1,KLEVS
    DO JL=KIDIA,KFDIA
      ...
      IF (LEVGEN) THEN
        ...
        ZLIQR(JL,JK)=MAX(RWRESTM3D(JL,JK),MIN(RWCAPM(JS),PWSAM1M(JL,JK)*(1.0_JPRB-ZF)))
      ELSE
...
!      residual and wilting point soil moisture (since it is common soil)
       ZWPBARE=(RWPWPM(JS)*(1.0_JPRB-ZBARE)+RWRESTM3D(JL,1_JPIM)*ZBARE)
       IF (JS >=1 ) THEN
