...
REAL(KIND=JPRB) ::  ZCOR, ZEPSF3, ZF, ZF1H, ZF1L, ZF2H, ZF2L, ZF2B, ZF21H, ZF21L, ZF21B, &
 ...
 & ZSALIN, ZWCAPM, ZWPWPM, ZWRESTM, ZROOTSA
...
!          FREEZING/MELTING OF SOIL WATER.
  ...
  DO JK=1,KLEVS
    DO JL=KIDIA,KFDIA
      ...
      IF (LEVGEN) THEN
        JS=KSOTY(JL)
        ZWCAPM=PSSDP3(JL,JK,SSDP3D_ID%NRWCAPM3D)
        ZWPWPM=PSSDP3(JL,JK,SSDP3D_ID%NRWPWPM3D)
        ZWRESTM=PSSDP3(JL,JK,SSDP3D_ID%NRWRESTM3D)
        ZLIQ(JL,JK)=MAX(ZWPWPM,MIN(ZWCAPM,PWSAM1M(JL,JK)*(1.0_JPRB-ZF)))
        ZLIQR(JL,JK)=MAX(ZWRESTM,MIN(ZWCAPM,PWSAM1M(JL,JK)*(1.0_JPRB-ZF)))
      ELSE
...
!   scaled leaf area index with cveg9= (rvcov)
    IF(PSSDP2(JL,SSDP2D_ID%NRVCOVL2D) /= 0.0_JPRB) THEN
     ZLAILSC=PLAIL(JL)/PSSDP2(JL,SSDP2D_ID%NRVCOVL2D)
    ELSE
     ZLAILSC=PLAIL(JL)
    ENDIF
    IF(PSSDP2(JL,SSDP2D_ID%NRVCOVH2D) /= 0.0_JPRB) THEN
     ZLAIHSC=PLAIH(JL)/PSSDP2(JL,SSDP2D_ID%NRVCOVH2D)
    ELSE
     ZLAIHSC=PLAIH(JL)
    ENDIF   
...
!           soil moisture stress function : F2
    ...
    ZWROOTH=0.0_JPRB
    DO JK=1,KLEVS
      ZWROOTL=ZWROOTL+ZLIQ(JL,JK)*PSSDP3(JL,JK,SSDP3D_ID%NRVROOTSAL3D)
      ZWROOTH=ZWROOTH+ZLIQ(JL,JK)*PSSDP3(JL,JK,SSDP3D_ID%NRVROOTSAH3D)
    ENDDO
    IF (LEVGEN) THEN
       JS=KSOTY(JL)
       ZWPWP=PSSDP3(JL,1_JPIM,SSDP3D_ID%NRWPWPM3D)
       IF (JS >= 1) THEN
         ZQWEVAP=1._JPRB/(PSSDP3(JL,1_JPIM,SSDP3D_ID%NRWCAPM3D)-ZWPWP)
       ELSE
         ZQWEVAP = 0.0_JPRB
       ENDIF
...
!      residual and wilting point soil moisture (since it is common soil)
       ZWPBARE=(PSSDP3(JL,1_JPIM,SSDP3D_ID%NRWPWPM3D)*(1.0_JPRB-ZBARE)+PSSDP3(JL,1_JPIM,SSDP3D_ID%NRWRESTM3D)*ZBARE)
       IF (JS >=1 ) THEN
          ZQWEVAPBARE=1._JPRB/(PSSDP3(JL,1_JPIM,SSDP3D_ID%NRWCAPM3D)-ZWPBARE)
       ELSE
          ZQWEVAPBARE=0._JPRB
       ENDIF
...
!   Use scaled LAI for canopy resistance computation
    ...
    PWETHS(JL)=PWETH(JL)
    ZRSMINB=PSSDP2(JL,SSDP2D_ID%NRVRSMINB2D)
...
!           radiation stress function (proposed by Alan Betts): ZF1 
    ...
    ZF1H=ZF1L
!           atmospheric moisture deficit stress function : F3
    ZHSTRL=PSSDP2(JL,SSDP2D_ID%NRVHSTRL2D)
    ZHSTRH=PSSDP2(JL,SSDP2D_ID%NRVHSTRH2D)
    ZQSAIR=FOEEW(PTMLEV(JL))/PAPHMS(JL)
    ZCOR=1.0_JPRB/(1.0_JPRB-RETV  *ZQSAIR)
    ZQSAIR=ZQSAIR*ZCOR
    ZF3L=EXP(-ZHSTRL*(ZQSAIR-PQMLEV(JL)))
...
!!                Compute weighted average of unfrozen soil water and F2
    DO JL=KIDIA,KFDIA  
      ...
      DO JK=1,KLEVS
        IF (KTILE == 4) THEN
          ZROOTSA=PSSDP3(JL,JK,SSDP3D_ID%NRVROOTSAL3D)
          ZWROOT(JL)=ZWROOT(JL)+ZLIQ(JL,JK)*ZROOTSA
        ELSEIF (KTILE==6 .OR. KTILE==7 ) THEN   
          ZROOTSA=PSSDP3(JL,JK,SSDP3D_ID%NRVROOTSAH3D)
          ZWROOT(JL)=ZWROOT(JL)+ZLIQ(JL,JK)*ZROOTSA
        ENDIF
      ENDDO
      IF (LEVGEN) THEN
...
!           bare ground fraction
      ...
      IF (LEVGEN) THEN
         JS=KSOTY(JL)
          ZWCAPM=PSSDP3(JL,1_JPIM,SSDP3D_ID%NRWCAPM3D)
          ZWPWP=PSSDP3(JL,1_JPIM,SSDP3D_ID%NRWPWPM3D)
!      bare ground evaporation stress is calculated with the weighted average of
!      residual and wilting point soil moisture (since it is common soil)
          ZWPBARE=(PSSDP3(JL,1_JPIM,SSDP3D_ID%NRWPWPM3D)*(1.0_JPRB-ZBARE)+PSSDP3(JL,1_JPIM,SSDP3D_ID%NRWRESTM3D)*ZBARE)
