...
DO JT=1,KTILES
  ...
  CASE(2)
   ...
   CASE(4)
    DO JL=KIDIA,KFDIA
      IF(PTSKM1M(JL,JT) > PTSRF(JL,JT) ) THEN
        PLAMSK(JL,JT) = PSSDP2(JL,SSDP2D_ID%NRVLAMSKSL2D)
      ELSE
        PLAMSK(JL,JT) = PSSDP2(JL,SSDP2D_ID%NRVLAMSKL2D)
      ENDIF
    ENDDO
  ...
  CASE(6)
    DO JL=KIDIA,KFDIA
      IF(PTSKM1M(JL,JT) > PTSRF(JL,JT) ) THEN
        PLAMSK(JL,JT) = PSSDP2(JL,SSDP2D_ID%NRVLAMSKSH2D)
      ELSE
        PLAMSK(JL,JT) = PSSDP2(JL,SSDP2D_ID%NRVLAMSKH2D)
      ENDIF
    ENDDO
