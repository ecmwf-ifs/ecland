...
!     Modifications
!     -------------
!     G. Arduini  2021-11-18: allow for snow over sea-ice 
...
REAL(KIND=JPRB),DIMENSION(KLON) :: ZTSURF5
REAL(KIND=JPRB) :: ZTSURF
...
! NOT ALL THE SNOW HAS BEEN MELTED
        PTSN5(JL) = ZT0
        PGSN5(JL) = (ZT0-ZTSURF5(JL))*ZRS5(JL)
      ELSE
! ALL THE SNOW HAS BEEN MELTED; COMPUTE TIME IT TOOK TO MELT
        PTSN5(JL) = ZT0
        PGSN5(JL) = ( ((ZT0-ZTSURF5(JL))*ZRS5(JL))*(ZDTM5(JL)+ZDTH5(JL))&
...
DO JL=KIDIA,KFDIA
  ...
  ELSE
    ...
    IF (ZTSTAR5(JL) <= ZT0) THEN
! NO MELTING OF SNOW
      ZRS = ZRS+(ZTSTAR5(JL)-ZTSURF5(JL))*PGSN(JL)
      ZTSTAR   = ZTSTAR+ZRS5(JL)*PGSN(JL)
      ZTSURF   = ZTSURF-ZRS5(JL)*PGSN(JL)
      PGSN(JL) = 0.0_JPRB
...
! MELTING OF SNOW:
      IF (ZSSTAR15(JL) >= 0.0_JPRB) THEN
! NOT ALL THE SNOW HAS BEEN MELTED
        ZRS = ZRS+(ZT0-ZTSURF5(JL))*PGSN(JL)
        ZTSURF = ZTSURF-ZRS5(JL)*PGSN(JL)
        PGSN(JL) = 0.0_JPRB
        PTSN(JL) = 0.0_JPRB
      ELSE
! ALL THE SNOW HAS BEEN MELTED; COMPUTE TIME IT TOOK TO MELT
        ZDTM = ZDTM+ZTMST*((ZT0-ZTSURF5(JL))*ZRS5(JL))*PGSN(JL)
        ZDTH = ZDTH+ZTMST*((ZT0-ZTSURF5(JL))*ZRS5(JL))*PGSN(JL)
        ZRS = ZRS+ZTMST*(ZT0-ZTSURF5(JL))*(ZDTM5(JL)+ZDTH5(JL))*PGSN(JL)
        ZTSURF = ZTSURF &
      ...
      ENDIF
    !**********************************************************!
    ! YOU HAVE TO MAKE THIS BIT FOR ZTSURF5 ISNTEAD OF PTSAM1M5
    !**********************************************************!
! MELTING PART OF THE TIME STEP
      ...
      ZRS = ZRS+(ZT0-ZTSURF5(JL))*ZGSN
      ZTSURF = ZTSURF-ZRS5(JL)*ZGSN
      ZDTH   = ZDTH-ZDTM
! HEATING PART OF THE TIME STEP
      ...
      ZRS    = ZRS+(ZT0-ZTSURF5(JL))*ZDTHI5(JL)*ZDTHI5(JL)*ZDTHI
      ZTSURF = ZTSURF-ZRS5(JL)*ZDTHI5(JL)*ZDTHI5(JL)*ZDTHI
    ENDIF
! SOLVE IMPLICIT TEMPERATURE EQUATION
    ZTSTARI = ZTSTARI+(PTSNM1M5(JL)+ZCONS25(JL) &
     & *(ZHFLUX5(JL)+ZHFLUXRF15(JL)+ZTSURF5(JL)*ZRS5(JL)))*ZTSTAR
    ...
    ZRS = ZRS+ZTSTARI5(JL)*ZCONS25(JL)*ZTSURF5(JL)*ZTSTAR
    ZTSURF = ZTSURF+ZTSTARI5(JL)*ZCONS25(JL)*ZRS5(JL)*ZTSTAR
...
!           IN THE GRID SQUARE
    ...
    IF (LDLAND(JL)) THEN
      PTSAM1M(JL,1)=PTSAM1M(JL,1)+ZTSURF
      ZTSURF=0._JPRB
    ELSE
      PTIAM1M(JL,1)=PTIAM1M(JL,1)+ZTSURF
      ZTSURF=0._JPRB
    ENDIF
  ENDIF
ENDDO
