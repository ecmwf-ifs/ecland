...
DO JK=1,KLEVSN
  ! INPUT LAYER DEPTHS
  ZDSN(KIDIA:KFDIA,JK) = PSSN(KIDIA:KFDIA,JK) / PRSN(KIDIA:KFDIA,JK)
  ! INPUT LAYER HEAT CONTENT
  ZHSN(KIDIA:KFDIA,JK) = ZIHCAP*(PSSN(KIDIA:KFDIA,JK))*&
                      (PTSN(KIDIA:KFDIA,JK)-ZRTT)-ZRLMLT*(PSSN(KIDIA:KFDIA,JK)-PWSN(KIDIA:KFDIA,JK))
ENDDO

 ! DEFINE VERTICAL GRID START AT 0 AT THE SNOW SURFACE IS POSITIVE DOWNWARDS
DO JK=0,KLEVSN
  ZDSNG(KIDIA:KFDIA,JK) = 0._JPRB
  ZDSNNEWG(KIDIA:KFDIA,JK) = 0._JPRB
ENDDO
DO JK=1,KLEVSN
  DO JL=KIDIA,KFDIA
...
DO JL=KIDIA,KFDIA
  ...
  IF ( LLREMAP(JL) ) THEN
    ...
    DO JK=1,KLEVSN
      DO JKK=1,KLEVSN
        ZREMAP(JL,JK,JKK)=0._JPRB
      ENDDO
    ENDDO

    DO JK=1,KLEVSN
      DO JKK=1,KLEVSN
        ZTMPL1(JL,JKK) = MAX(ZDSNNEWG(JL,JK-1),ZDSNG(JL,JKK-1)) ! TOP BOUNDARY
        ZTMPL2(JL,JKK) = MIN(ZDSNNEWG(JL,JK),ZDSNG(JL,JKK))     ! BOTTOM BOUNDARY
        ZREMAP(JL,JK,JKK) = MAX(0._JPRB,ZTMPL2(JL,JKK)-ZTMPL1(JL,JKK))
      ENDDO
...
! INPUT LAYER HEAT CONTENT
DO JK=1,KLEVSN
  ZHSNNEW(KIDIA:KFDIA,JK) = ZIHCAP*(PSSNNEW(KIDIA:KFDIA,JK))*&
                      (PTSNNEW(KIDIA:KFDIA,JK)-ZRTT)-ZRLMLT*(PSSNNEW(KIDIA:KFDIA,JK)-PWSNNEW(KIDIA:KFDIA,JK))
ENDDO
