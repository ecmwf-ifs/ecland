...
DO JK=1,KLEVSN
  ZLEVMAX(JK)    = YDSOIL%RLEVSNMAX(JK)
  ZLEVMIN(JK)    = YDSOIL%RLEVSNMIN(JK)
  ZLEVMAX_GL(JK) = YDSOIL%RLEVSNMAX_GL(JK)
  ZLEVMIN_GL(JK) = YDSOIL%RLEVSNMIN_GL(JK)
ENDDO
...
IF (NSNMLWS == 3_JPIM) THEN
  DO JK=1,KLEVSN
    ZSNDEPTH(KIDIA:KFDIA, JK)     = 0.5_JPRB*ZDSN(KIDIA:KFDIA, JK)
    ZSNDEPTHREAL(KIDIA:KFDIA, JK) = 0.5_JPRB*ZDSNREAL(KIDIA:KFDIA, JK)
  ENDDO
ELSE
...
DO JK=1,KLEVSN
  ZTSNWS(KIDIA:KFDIA,JK)   = ZTSN(KIDIA:KFDIA)
  ZRSNWS(KIDIA:KFDIA,JK)   = ZRSN(KIDIA:KFDIA)
  ZWSNWS(KIDIA:KFDIA,JK)   = 0._JPRB
ENDDO
ZSSNWS(KIDIA:KFDIA,1)      = ZSSN(KIDIA:KFDIA)
DO JK=2,KLEVSN
  ZSSNWS(KIDIA:KFDIA,JK)   = 0._JPRB
ENDDO
DO JK=1,KLEVSN
  ZTCONSTAVG(KIDIA:KFDIA,JK)=0._JPRB
  ZTCONSTSTD(KIDIA:KFDIA,JK)=0._JPRB
  ZRCONSTAVG(KIDIA:KFDIA,JK)=0._JPRB
  ZRCONSTSTD(KIDIA:KFDIA,JK)=0._JPRB
ENDDO
