...
IF (INT(MYPROC) == 1) THEN
  NPROC_CMF=MPL_NPROC(ICOMM_CMF)
ENDIF
CALL MPL_BROADCAST(NPROC_CMF, KROOT=1, KTAG=100, CDSTRING='MASTER1S:NPROC_CMF')

IF (INT(MYPROC) .LE. MIN(INT(NPROC),NPROC_CMF)) THEN
