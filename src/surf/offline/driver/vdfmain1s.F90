...
SUBROUTINE VDFMAIN1S  ( CDCONF, &
 ...
 & PCIL, &
 & PLAILP, PLAIHP, PAVGPAR, PSNM1M , PRSNM1M , &
...
USE YOMRIP0  , ONLY : NINDAT !for new bvoc emission module
...
!  ! Identify current Day of Year:
!  ! Start Jul day + number of days since start of model 
!IYEAR0=NCCAA(NINDAT)
!IMONTH0=NMM(NINDAT)
!IDAY0=NDD(NINDAT)
!  ! ...current date...
!CALL UPDCAL (IDAY0,IMONTH0,IYEAR0,YDMODEL%YRML_GCONF%YRRIP%NSTADD,IDAY,IMONTH,IYEAR,ILMONTHS,-1)
!  ! ...day of year, number of days in year 
!ZDOY=(RJUDAT(IYEAR,IMONTH,IDAY) - RJUDAT(IYEAR,1,1))+1._JPRB 


!  !setup photosynthetic photon flux density at the top of atmosphere,
!  !required for BVOC emissions module
!ZPPFD_TOA=3000._JPRB + 99._JPRB*COS(2.0_JPRB*RPI * (ZDOY-10._JPRB) /365._JPRB )
...
! input data, non-tiled
 ...
 !& PLAILP=ZLAILP, PLAIHP=ZLAIHP, PAVGPAR=ZAVGPAR, &
 !& VBVOCLAIL=ZLAILP, VBVOCLAIH=ZLAIHP, PAVGPAR=ZAVGPAR, &   
 & PLAILP=PLAILP, &
 & PLAIHP=PLAIHP, &
 & PAVGPAR=PAVGPAR, &
