...
SUBROUTINE VDFMAIN1S  ( &
 & YDSURF , KIDIA  , KFDIA  , KLON   , KLEV   , KLEVS  , KSTEP  , KTILES , KVTYPES, KDIAG, &
...
REAL(KIND=JPRB) ::    ZPSNM1(KLON)      , ZPRSN1(KLON)      ,&
                    & ZPTSM1(KLON)      , ZPTSNOW1(KLON)
...
CALL SURFEXCDRIVER(&
 & YDSURF=YDSURF, KIDIA=KIDIA, KFDIA=KFDIA, KLON=KLON, KLEVS=KLEVS, KTILES=KTILES, KVTYPES=KVTYPES, KDIAG=KDIAG, &
...
DO JL=KIDIA,KFDIA
  ZPSNM1(JL)=PSNM1M(JL,1)
  ZPRSN1(JL)=PRSNM1M(JL,1)
  ZPTSM1(JL)=PTSAM1M(JL,1)
  ZPTSNOW1(JL)=PTSN(JL,1)
ENDDO
...
ENDIF
!$loki end remove
CALL VDFDIFH1S (YDSURF  , KIDIA  , KFDIA  , KLON   , KLEV   , ITOP   , KTILES, KTVL, KTVH, KLEVSN, &
            ...
            & ZCPTSTIU,ZCAIRTIU,ZCSATTIU,ZTSRF,ZLAMSK,ZPSNM1,ZPRSN1, &
            & ZDQSTI , PTSKTI , PTSKRAD, ZPTSM1    , &
            & ZPTSNOW1 , PTICE  , PSST, &
...
CALL SURFPP( YDSURF=YDSURF,KIDIA=KIDIA,KFDIA=KFDIA,KLON=KLON,KTILES=KTILES, &
