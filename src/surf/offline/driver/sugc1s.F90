...
INTEGER(KIND=JPIM) :: ICT,JLA,JLO,JK,J,JVT,JTILE,IL,IBL
...
!             -----------------------------------
  ...
  GELAT(1,1)=RLAT*RPI/180.
  GELAM(1,1)=RLON*RPI/180.

  IF (GELAM(1,1) < 0.) THEN
    GELAM(1,1)=GELAM(1,1)+2.*RPI
  ENDIF
...
DO ICT = 1,NPOI
  IBL=MAX((ICT-1)/NPROMA,1)
  IF(IBL == 1)THEN
    IL = ICT
  ELSE
    IL=MOD(ICT,(IBL-1)*NPROMA)
  ENDIF
  ...
  RCORI(ICT)=2.*ROMEGA*GEMU(IL,IBL)
ENDDO
