...
!    &           ,PSSDP2, PTVL   ,PCO2TYP,PTVH   ,PSOTY  ,PSDOR,PCVL   ,PCVH  ,PCUR &
   ...
   & PTVH=PSURF%GSD%PTVH(:,IBL), PSOTY=PSURF%GSD%PSOTY(:,IBL),PSDOR=PSURF%PHSTD(:,IBL),PCVLC=PSURF%GSD%PCVL(:,IBL), &
   & PCVHC=PSURF%GSD%PCVH(:,IBL),PCURC=PSURF%GSD%PCUR(:,IBL),&
 !    &           ,PLAILC ,PLAIHC, PLAILI, PLAIHI, PLAILC, PLAIHC,ZAVGPARC &    
   &  PLAILC=PSURF%GSD%PLAILC(:,IBL), PLAIHC=PSURF%GSD%PLAIHC(:,IBL),&
   &  PLAILI=    SURFL%PLAILI(:,IBL), PLAIHI=    SURFL%PLAIHI(:,IBL),&
   & PLAILCP=PSURF%GSD%PLAILC(:,IBL),PLAIHCP=PSURF%GSD%PLAIHC(:,IBL),&
   & PAVGPARC=ZAVGPARC(:,IBL),&
 !   &           ,PLSM   ,PCI    ,PCIL, PCLAKE,PHLICE,PGEMU  ,PSNS, PWL,PRSN & 
   & PLSM=PSURF%GSD%PLSM(:,IBL),PCI=PSURF%GSD%PCI(:,IBL),PCIL=PSURF%GSD%PCIL(:,IBL),&
   & PCLAKE=PSURF%GSD%PCLAKE(:,IBL),PHLICE=PSURF%GSP%PHLICE(:,IBL),&
   & PGEMU=PAUX%PGEMU(:,IBL),PSNM1M=SURFL%PSNS(:,:,IBL),PWLM1M=PSURFL%PWL(:,IBL),PRSNM1M=SURFL%PRSN(:,:,IBL),
 !    &           ,LESNICE &
   & LESNICE=LESNICE,&
 !    &           ,LLLAND ,LLSICE ,LLLICE, LLLAKE,LLNH, LLOCN_KPP &   
 ...
 !    &           ,ZCVL   ,ZCVH   ,ZCUR  , ZLAIL , ZLAIH, ZLAILP, ZLAIHP, ZAVGPAR & 
   & PCVL=ZCVL(:,IBL),PCVH=ZCVH(:,IBL),PCUR=ZCUR(:,IBL),PLAIL=ZLAIL(:,IBL),PLAIH=ZLAIH(:,IBL),&
   & PLAILP=ZLAILP(:,IBL),PLAIHP=ZLAIHP(:,IBL),&
   & PAVGPAR=ZAVGPAR(:,IBL), &
 !    &           ,ZWLMX  ,ZFRTI,ZCSN)
   & PWLMX=ZWLMX(:,IBL),PFRTI=ZFRTI(:,:,IBL), PCSN=ZCSN(:,IBL))
...
!     &     , LESNICE=LESNICE)
    ...
    CALL SURFRAD &
     ...
     &     , PALBF=ZSURF_GSD_VF_PALBF(:,IBL) ,PALBICEF=PALBICEF(:,IBL),PTVH=PSURF%GSD%PTVH(:,IBL) &
     &     , PALCOEFF=ZALCOEFF(:,:,IBL) ,PCUR=PSURF%PCUR(:,IBL), PCVH=PSURF%PCVH(:,IBL) &
     &     , PASN=ZSURF%PASN(:,:,IBL)   , PMU0=ZAUX_PMU0M(:,IBL)  , PTS=ZSURF_GSP_RR_PT_T9PTL(:,IBL)    , PWND=ZAUXL_ZWND(:,IBL) &
     &     , PWS1=ZSURF_GSP_SB_PQ_T9_ZWS1(:,1,IBL), KSOTY=ZSURF_ISOTY(:,IBL), PFRTI=ZSURFL_ZFRTI(:,:,IBL),PHLICE=PSURF%GSP%PHLICE(:,IBL),PTLICE=ZSURF_GSP_SL_PLICT_T9PTLICE(:,IBL) &
...
!             ,PTSN, PSNS, PRSN, PWSN                           )
  ...
  CALL SURFWS(YDSURF=YSURF, KIDIA, KFDIA, KLON, KLEVS, KLEVSN, KTILES  &
             ,PSDOR=PSURF%PHSTD(:,IBL),LDSICE=LLKEYS%LLSICE(:,IBL)                                      &
             ,PLSM=PSURF%GSD%PLSM(:,IBL), PCIL=PSURF%GSD%PCIL(:,IBL), PFRTI=SURFL%ZFRTI, PMU0=PAUX%PMU0M                          &
             ,PTSAM1M=PSURF%PTSA, PTSKIN=PSURF%PTL,PALBSN=PSURF%PASN(:,:,IBL)                                    &
             ,PTSNM1M=PSURF%PTSN(:,:,IBL), PSNM1M=SURFL%PSNS(:,:,IBL), PRSNM1M=SURFL%PRSN(:,:,IBL), PWSNM1M=SURFL%PWSN(:,:,IBL)                           )
...
! & PDHTLS,PDHTSS,PDHTTS,PDHTIS,PDHCO2S,PDHBVOCS,PDHVEGS)
 ...
 & PCIL=PSURF%GSD%PCIL(:,IBL), &
 & PSNM1M=SURFL%PSNS(:,:,IBL),PRSNM1M=SURFL%PRSN(:,:,IBL), &
 & STATE%PU  ,STATE%PV  ,STATE%PT  ,STATE%PQ  ,STATE%PC  ,&
 & PAPRS,PAUX%PGEOM1,&
 & PTL,PTSA,PWSA,&
 & FLUX%PFRSO(1,KLEV),FLUX%PFRTH(1,KLEV),PSURF%ZEMIS,&
 & PTSN=PSURF%PTSN(:,:,IBL), PSURF%PTIA(:,1),&
 & PHLICE=PSURF%GSP%PHLICE(:,IBL),PSURF%PTLICE,PSURF%PTLWML, &                             !FLAKE
...
!!-END OF CALL SURFTSTP
 ...
 !   & PSNS  ,PTSN,PASN,PRSN,PWSN,ZPAPSRF,PTL,PTSA,PTIA,&    
 !   & PWL  ,PWSA,&
   &  PSNM1M=PSURFL%PSNS(:,:,IBL) ,PTSNM1M=PSURF%PTSN(:,:,IBL), PASNM1M=PSURF%PASN(:,:,IBL), &
   &  PRSNM1M=SURFL%PRSN(:,:,IBL), PWSNM1M=SURFL%PWSN(:,:,IBL), &
   &  PAPRS=PAUX%PAPRS(:,KLEV,IBL), PTSKM1M=PSURF%GSP_RR_PT_T9(:,IBL),&
   &  PTSAM1M=PSURF%GSP_SB_PT_T9(:,:,IBL), PTIAM1M=PSURF%GSP_SB_PTL_T9(:,:,IBL),&
   &  PWLM1M=PSURFL%PWL(:,IBL), PWSAM1M=PSURF%GSP_SB_PQ_T9(:,:,IBL),&
 !    & PTLICE,PTLMNW,PTLWML,PTLBOT,PTLSF, &                   !FLAKE
 ...
 !    & PHLICE,PHLML,PGEMU,PLDEPTH,LLLAKE,PCLAKE, &            !FLAKE
   ...
   &  PLDEPTH=PSURF%PDL(:,IBL),LDLAKE=LLKEYS%LDLAKE(:,IBL), PCLAKE=PSURF%GSD%PCLAKE(:,IBL),&!LAKE
 !    & PFPLCL  , PFPLSL,&
 !    & ZFRTH   , PFPLCN  , PFPLSN,&
 !    & ZCVL    , ZCVH    , PCUR, ZWLMX   , ZEVAPSNW,&
 !    & PU,PV,PT,&
   ...
   &  PCVL=ZCVL(:,IBL), PCVH=ZCVH(:,IBL), PCUR=PSURF%PCUR(:,IBL), &
   &  PWLMX=ZWLMX(:,IBL) , PEVAPSNW=ZEVAPSNW(:,IBL),&
