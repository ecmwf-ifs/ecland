SUBROUTINE CALLPAR1S_TEST (CDCONF &
...
! - INPUT .
     & , LLKEYS &
! - OUTPUT .
     & , PSURF, PSURFL, PAUX, PAUXL, PFLUX, STATE, GEMSL, PDDHS & 
!-----------------------------------------------------------------------
! - INPUT .
...
!     &     , PASN=PASN   , PMU0=PMU0M  , PTS=PTL    , PWND=ZWND(:,IBL) &
...
!     &     , PASN=PASN   , PMU0=PMU0M  , PTS=PTL    , PWND=PAUXL%ZWND(:,IBL) &
!     &     , PWS1=ZWS1, KSOTY=ISOTY, PFRTI=ZFRTI,PHLICE=PHLICE,PTLICE=PTLICE &
!     &     , PALBD=ZALBD(:,1:NSW)  , PALBP=ZALBP(:,1:NSW)  , PALB=PALB &
...
! & KDHVCO2S,KDHFCO2S, KDHVBVOCS, KDHVVEGS,KDHFVEGS,  &
 ...
 & KTRAC=KTRAC, KLEVSN=KLEVSN, KLEVI=KLEVI, LDLAND=LLLAND, &
...
! & PSNS,PRSN, &
 ...
 & PCUR=PSURF%GSD%PCUR(:,IBL),PFWET=PSURF%PFWET(:,IBL),PLAT=PAUX%PLAT(:,IBL),PLAIL=PSURFL%ZLAIL(:,IBL),PLAIH=PSURFL%ZLAIH(:,IBL),&
...
! & PU  ,PV  ,PT  ,PQ  ,PC  ,&
! & PAPRS,PGEOM1,&
 & PUM1=STATE%PU(:,KLEV,IBL) ,PVM1=STATE%PV(:,KLEV,IBL)  ,PTM1=STATE%PT(:,KLEV,IBL)  ,&
 & PQM1=STATE%PQ(:,KLEV,IBL)  ,PCM1=GEMSL%PC(:,KLEV,IBL)  ,&
 & PAPHM1=PAUX%PAPRS(:,KLEV,IBL),PGEOM1=PAUX%PGEOM1(:,KLEV,IBL),&
! & PTL,PTSA,PWSA,&
! & PFRSO(1,KLEV),PFRTH(1,KLEV),ZEMIS,&
...
! & PUSTRTI,PVSTRTI,PAHFSTI,PEVAPTI,PTSKTI,PSLRFLTI,&
 ...
 & PEVAPTI=PSURF%PEVAPTI(:,:,IBL), PTSKTI=PSURF%PTSKTI(:,:,IBL) ,PSLRFLTI=PSURFL%ZAHFTRTI(:,:,IBL), &
 !-UPDATED FIELDS FOR VEGETATION TYPES
! & PANDAYVT,PANFMVT,& 
...
!!-END OF CALL SURFTSTP
 ...
 !    & PAHFSTI, PEVAPTI, ZFRSOTI, PSLRFLTI,&
   ...
   &  PSSRFLTI=PSURFL%ZFRSOTI(:,:,IBL),PSLRFLTI=PSURFL%ZAHFTRTI(:,:,IBL),&
 !   & GELAT, PANFMVT, PANDAYVT, PMU0M,&                      !CTESSEL
 !   & ZCVT, ZLAIVT, ZLAIL, ZLAIH, &                          !CTESSEL
 !   & PCIL, &
 !   & PLAILC,  PLAIHC, &
   &  PLAT=PAUX%PLAT(:,IBL),PANFMVT=PSURFL%PANFMVT(:,:,IBL), PANDAYVT=PSURFL%PANDAYVT(:,:,IBL), PMU0=PAUX%PMU0M(:,IBL),&!CTESSEL    
   &  PCVT=PSURFL%ZCVT(:,:,IBL), PLAIVT=PSURFL%ZLAIVT(:,:,IBL),  &
   & PLAIL=PSURF%ZLAIL(:,IBL), PLAIH=PSURF%ZLAIH(:,IBL),&!CTESSEL
   &  PCIL=PSURF%PCIL(:,IBL), &
   &  PLAILC=PSURF%GSD%PLAILC(:,IBL), PLAIHC=PSURF%GSD%PLAIHC(:,IBL),&
 !   & LLLAND, LLSICE,LLLICE, LESICE, LLNH, LLOCN_KPP,&              !KPP
 ...
 !   & PWL  ,PWSA,&
   ...
   &  PAPRS=PAUX%PAPRS(:,KLEV,IBL), PTSKM1M=PSURF%GSP%PTL(:,IBL),&
