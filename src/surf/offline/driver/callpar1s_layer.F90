...
!CALL SURFBC(YSURF, KIDIA,KFDIA,KLON,KTILES,KLEVSN &
  CALL SURFBC (YURF=YSURF,KIDIA=KIDIA,KFDIA=KFDIA,KLON=KLON,KTILES=KTILES,KLEVSN=KLEVSN,&
!    &           ,PSSDP2, PTVL   ,PCO2TYP,PTVH   ,PSOTY  ,PSDOR,PCVL   ,PCVH  ,PCUR &
   & PSSDP2=PSURF%GSD_S2_PGROUP(:,:,IBL), PTVL=PSURF%GSD_VF_PTVL(:,IBL),PCO2TYP=PSURF%GSD_VF_PCO2TYP(:,IBL),&
   & PTVH=PSURF%GSD_VF_PTVH(:,IBL), PSOTY=PSURF%GSD_VF_PSOTY(:,IBL),PSDOR=PSURF%PHSTD(:,IBL),PCVLC=PSURF_GSD_VF_PCVL(:,IBL), &
   & PCVHC=PSURF%GSD_VF_PCVH(:,IBL),PCURC=PSURF%GSD_VF_PCUR(:,IBL),&
 !    &           ,PLAILC ,PLAIHC, PLAILI, PLAIHI, PLAILC, PLAIHC,ZAVGPARC &    
   & PLAILC=PSURF%GSD_VF_PLAIL(:,IBL),PLAIHC=PSURF%GSD_VF_PLAIH(:,IBL),PLAILI=SURFL%ZLAILI(:,IBL),PLAIHI=SURFL%ZLAIHI(:,IBL),&
   & PLAILCP=PSURF%GSD_VF_PLAILP(:,IBL),PLAIHCP=PSURF%GSD_VF_PLAIHP(:,IBL),&
   & PAVGPARC=PSURF%GSD_VF_PAVGPAR(:,IBL),&
 !   &           ,PLSM   ,PCI    ,PCIL, PCLAKE,PHLICE,PGEMU  ,PSNS, PWL,PRSN & 
   & PLSM=PSURF%GSD_VF_PLSM(:,IBL),PCI=PSURF%GSD_VF_PCI(:,IBL),PCIL=PSURF%GSD_VF_PCIL(:,IBL),&
   & PCLAKE=PSURF%GSD_VF_PCLK(:,IBL),PHLICE=PSURF%GSP_SL_PLICD_T0(:,IBL),&
   & PGEMU=PAUX%PGEMU(:,IBL),PSNM1M=SURFL%ZSNM1M(:,:,IBL),PWLM1M=SURFL%ZWLM1M(:,IBL),PRSNM1M=SURFL%ZRSNM1M(:,:,IBL),
 !    &           ,LESNICE &
   & LESNICE=LESNICE,&
 !    &           ,LLLAND ,LLSICE ,LLLICE, LLLAKE,LLNH, LLOCN_KPP &   
 ...
 !    &           ,ZCVL   ,ZCVH   ,ZCUR  , ZLAIL , ZLAIH, ZLAILP, ZLAIHP, ZAVGPAR & 
   & PCVL=PSURF%PCVL(:,IBL),PCVH=PSURF%PCVH(:,IBL),PCUR=PSURF%PCUR(:,IBL),PLAIL=PSURF%PLAIL(:,IBL),PLAIH=PSURF%PLAIH(:,IBL),&
   & PLAILP=PSURF%PLAILP(:,IBL),PLAIHP=PSURF%PLAIHP(:,IBL),&
   & PAVGPAR=PSURF%PAVGPAR(:,IBL), &
 !    &           ,ZWLMX  ,ZFRTI,ZCSN)
   & PWLMX=SURFL%ZWLMX(:,IBL),PFRTI=SURFL%ZFRTI(:,:,IBL), PCSN=PSURF%GSD_VD_PSCF(:,IBL))
...
!     &     , LESNICE=LESNICE)
    ...
    CALL SURFRAD &
     &     ( YSURF,KDD=IDD, KMM=IMM, KMON=IMON  , KSECO=ISEC0 &
     &     , KIDIA=KIDIA  , KFDIA=KFDIA  , KLON=KLON   , KTILES=KTILES , KSW=NSW, KLW=NLWEMISS &
     &     , LDNH=ZLLKEYS_LLNH(:,IBL),LDLAND=ZLLKEYS_LLLAND(:,IBL) &
     &     , PSSDP3=ZSURF_GSD_S3_PSSDP3(:,:,:,IBL) &
     &     , PALBF=ZSURF_GSD_VF_PALBF(:,IBL) ,PALBICEF=PALBICEF(:,IBL),PTVH=PTVH(:,IBL) &
     &     , PALCOEFF=ZALCOEFFZSURF_GSD_VF_PGROUP(:,IALSTART:IALEND,IBL) ,PCUR=ZSURF_GSD_VF_PCUR(:,IBL), PCVH=ZSURF_GSD_VF_PCVH(:,IBL) &
     &     , PASN=ZSURF_GSP_SG_PA_T9PASN(:,1,IBL)   , PMU0=ZAUX_PMU0M(:,IBL)  , PTS=ZSURF_GSP_RR_PT_T9PTL(:,IBL)    , PWND=ZAUXL_ZWND(:,IBL) &
     &     , PWS1=ZSURF_GSP_SB_PQ_T9_ZWS1(:,1,IBL), KSOTY=ZSURF_ISOTY(:,IBL), PFRTI=ZSURFL_ZFRTI(:,:,IBL),PHLICE=ZSURF_GSP_SL_PLICD_T9PHLICE(:,IBL),PTLICE=ZSURF_GSP_SL_PLICT_T9PTLICE(:,IBL) &
     &     , PALBD=ZSURFL_ZALBD(:,1:NSW,IBL)  , PALBP=ZSURFL_ZALBP(:,1:NSW,IBL)  , PALB=ZSURF_GSD_VD_PALB(:,IBL) &
     &     , PSPECTRALEMISS=ZSURFL_ZSPECTRALEMISS(:,1:NLWEMISS,IBL) , PEMIT=ZSURF_ZEMIS(:,IBL) &
     &     , PALBTI=ZSURFL_ZALBTI(:,:,IBL), PCCNL=ZAUXL_ZCCNL(:,IBL), PCCNO=ZAUXL_ZCCNO(:,IBL) &
     &     , LNEMOLIMALB=LNEMOLIMALB&
     &     , LESNICE=LESNICE)
...
!             ,PTSN, PSNS, PRSN, PWSN                           )
  ...
  CALL SURFWS(YDSURF=YSURF, KIDIA, KFDIA, KLON, KLEVS, KLEVSN, KTILES  &
             ,PSDOR=PSURF%PHSTD,LDSICE=LLKEYS%LLSICE                                      &
             ,PSURF%PLSM, PSURF%PCIL, PFRTI=SURFL%ZFRTI, PMU0=PAUX%PMU0M                          &
             ,PTSAM1M=PSURF%PTSA, PTSKIN=PSURF%PTL,PALBSN=PSURF%PASN                                    &
             ,PTSNM1M=PTSN, PSNM1M=PSNS, PRSNM1M=PRSN, PWSNM1M=PWSN                           )
...
! & PDHTLS,PDHTSS,PDHTTS,PDHTIS,PDHCO2S,PDHBVOCS,PDHVEGS)
 ...
 & LESNICE,&
 & PMU0M   , &
 & PTSPHY,PSURF%ITVL,PSURF%ICO2TYP,PSURF%ITVH,PSURF%ZCVL,PSURF%ZCVH,PSURF%PCUR,PFWET,PLAT,PSURF%ZLAIL,PSURF%ZLAIH,&
 & PSURF%ZLAILP, PSURF%ZLAIHP, PSURF%ZAVGPAR,PSURF%PISOP_EP, &
 & PCIL, &
 & PSNS,PRSN, &
 & STATE%PU  ,STATE%PV  ,STATE%PT  ,STATE%PQ  ,STATE%PC  ,&
 & PAPRS,PAUX%PGEOM1,&
 & PTL,PTSA,PWSA,&
 & FLUX%PFRSO(1,KLEV),FLUX%PFRTH(1,KLEV),PSURF%ZEMIS,&
 & PSURF%PTSN, PSURF%PTIA(:,1),&
 & PSURF%PHLICE,PSURF%PTLICE,PSURF%PTLWML, &                             !FLAKE
 & PSURF%PSST  ,PSURF%ISOTY, SURFL%ZFRTI , SURFL%ZALBTI,SURFL%ZWLMX,&
 & PSURF%ZCHAR , PSURF%ZCHARHQ, PSURF%ZUCURR , PSURF%ZVCURR,&
 & AUXL%PTL,&                         ! skin temp at latest rad timestep
 & GEMSL%PCFLX,&
 & PSURF%PSSDP2, PSURF%PSSDP3,&
 !-OUTPUT
 & GEMSL%PZ0M ,GEMSL%PZ0H ,&
 & PDIAG%PVDIS , FLUX%PFTLHEV,&
 & FLUX%PFTLHSB,&
 & FLUX%PFWSB,&
 & PSURF%PU10M ,PSURF%PV10M ,PSURF%PT2M  ,PSURF%PD2M ,PSURF%PQ2M ,PSURF%PBLH ,SURFL%ZFRSOTI,SURFL%ZEVAPSNW,&
 & ZEXDIAG,PGUST, PZIDLWV,&
 !-UPDATED TENDENCIES
 & TENDENCY_LOC%ZTE   ,TENDENCY_LOC%ZQE   ,TENDENCY_LOC%ZVOM  ,TENDENCY_LOC%ZVOL  ,GEMSL%ZTENC ,PSURF%PTLE1,&
 !-UPDATED FIELDS FOR TILES
 & PSURF%PUSTRTI,PSURF%PVSTRTI,PSURF%PAHFSTI,PSURF%PEVAPTI,PSURF%PTSKTI,SURFL%PSLRFLTI,&
 !-UPDATED FIELDS FOR VEGETATION TYPES
 & SURFL%PANDAYVT,SURFL%PANFMVT,& 
 !-OUTPUT FLUXES
 & PEVAPTIU,FLUX%PDIFTS,FLUX%PDIFTQ,FLUX%PSTRTU,FLUX%PSTRTV,ZKH,&
 & PAN,PSURF%PAG,PRD,PRSOIL_STR,PSURF%PRECO,PSURF%PCO2FLUX,PSURF%PCH4FLUX,& 
 & GEMSL%PBVOCFLUX, &
 ! DDH OUTPUTS
 & PDDHS%PDHTLS,PDDHS%PDHTSS,PDDHS%PDHTTS,PDDHS%PDHTIS,PDDHS%PDHCO2S,PDDHS%PDHBVOCS,PDDHS%PDHVEGS)
...
!!-END OF CALL SURFTSTP
 ...
 !   & KDHVBIOS, KDHFBIOS, KDHVVEGS, KDHFVEGS, &              !CTESSEL
   ...
   &  KDHVVEGS=IKDHVVEGS, KDHFVEGS=IKDHFVEGS,&!CTESSEL
     !&  KVTYPES=IKVTYPES, & !!!!!Doesn't exist in 50R1
 !   & ITVL  , ITVH, IVEG, ISOTY, &
   &  KTVL=PSURF%ITVL(:,IBL) ,KTVH=PSURF%ITVH(:,IBL), KVEG=IVEG(:,:,IBL),KSOTY=PSURF%KSOTY(:,IBL),& !Why KVEG not in DT!!!
 !    & PSSDP2, PSSDP3, &
   &  PSSDP2=PSURF_GSD_S2(:,:,IBL), PSSDP3=PSURF_GSD_S3(:,:,:,IBL), &
 !    & PTSPHY , PSDOR , ZFRTI,&
 !    & PSST, PUSTRTI, PVSTRTI,&
 !    & PAHFSTI, PEVAPTI, ZFRSOTI, PSLRFLTI,&
   &  PTSPHY=PTSPHY,PSDOR=PSURF%PSDOR(:,IBL), PFRTI=PSURFL%PFRTI(:,:,IBL),&
   &  PSST=PSURF%PSST(:,IBL), PUSTRTI=PSURF%PUSTRTI(:,:,IBL), PVSTRTI=PSURF%PVSTRTI(:,:,IBL),&
   &  PAHFSTI=PSURF%PAHFSTI(:,:,IBL), PEVAPTI=PSURF%PEVAPTI(:,:,IBL), &
   &  PSSRFLTI=PSURFL%PSSRFLTI(:,:,IBL),PSLRFLTI=PSURFL%PSLRFLTI(:,:,IBL),&
 !   & GELAT, PANFMVT, PANDAYVT, PMU0M,&                      !CTESSEL
 !   & ZCVT, ZLAIVT, ZLAIL, ZLAIH, &                          !CTESSEL
 !   & PCIL, &
 !   & PLAILC,  PLAIHC, &
   &  PLAT=PAUX%PLAT(:,IBL),PANFMVT=PSURFL%PANFMVT(:,:,IBL), PANDAYVT=PSURFL%PANDAYVT(:,:,IBL), PMU0=PAUX%PMU0(:,IBL),&!CTESSEL    
   &  PCVT=ZCVT(:,:,IBL), PLAIVT=ZLAIVT(:,:,IBL), PLAIL=PSURF%PLAIL(:,IBL), PLAIH=PSURF%PLAIH(:,IBL),&!CTESSEL
   &  PCIL=PSURF%PCIL(:,IBL), &
   &  PLAILC=PSURF%PLAILC(:,IBL), PLAIHC=PSURF%PLAIHC(:,IBL),&
 !   & LLLAND, LLSICE,LLLICE, LESICE, LLNH, LLOCN_KPP,&              !KPP
   &  LDLAND=LLKEYS%LDLAND(:,IBL), LDSICE=LLKEYS%LDSICE(:,IBL), LDLICE=LLKEYS%LDLICE(:,IBL), LDSI=LESICE, LDNH=LLKEYS_LDNH(:,IBL), &
   &  LDOCN_KPP=LLKEYS_LDOCN_KPP(:,IBL),&
 !   & PSNS  ,PTSN,PASN,PRSN,PWSN,ZPAPSRF,PTL,PTSA,PTIA,&    
 !   & PWL  ,PWSA,&
   &  PSNM1M=PSURFL%PSNM1M(:,:,IBL) ,PTSNM1M=PSURF%GSP_SG_PT_T9(:,:,IBL), PASNM1M=PSURF%GSP_SG_PA_T9(:,1,IBL), &
   &  PRSNM1M=PSURF%GSP_SG_PR_T9(:,:,IBL), PWSNM1M=PSURF%GSP_SG_PW_T9(:,:,IBL),
   &  PAPRS=PAUX%PAPRS(:,KLEV,IBL), PTSKM1M=PSURF%GSP_RR_PT_T9(:,IBL),&
   &  PTSAM1M=PSURF%GSP_SB_PT_T9(:,:,IBL), PTIAM1M=PSURF%GSP_SB_PTL_T9(:,:,IBL),&
   &  PWLM1M=PSURFL%PWLM1M(:,IBL), PWSAM1M=PSURF%GSP_SB_PQ_T9(:,:,IBL),&
 !    & PTLICE,PTLMNW,PTLWML,PTLBOT,PTLSF, &                   !FLAKE
   &  PTLICEM1M=PSURF%GSP_SL_PLICT_T9(:,IBL),PTLMNWM1M=PSURF%GSP_SL_PLTLT_T9(:,IBL),&!LAKE
   &  PTLWMLM1M=PSURF%GSP_SL_PLMLT_T9(:,IBL),PTLBOTM1M=PSURF%GSP_SL_PLBLT_T9(:,IBL),&!LAKE
   &  PTLSFM1M=PSURF%GSP_SL_PLSHF_T9(:,IBL),&!LAKE
 !    & PHLICE,PHLML,PGEMU,PLDEPTH,LLLAKE,PCLAKE, &            !FLAKE
   &  PHLICEM1M=PSUR%GSP_SL_PLICD_T9(:,IBL),PHLMLM1M=PSURF%GSP_SL_PLMLD_T9(:,IBL),&!LAKE
   &  PGEMU=PAUX%PGEMU(:,IBL),&
   &  PLDEPTH=PSURF%PDL(:,IBL),LDLAKE=LLKEYS%LDLAKE(:,IBL), PCLAKE=PSURF%PCLK(:,IBL),&!LAKE
 !    & PFPLCL  , PFPLSL,&
 !    & ZFRTH   , PFPLCN  , PFPLSN,&
 !    & ZCVL    , ZCVH    , PCUR, ZWLMX   , ZEVAPSNW,&
 !    & PU,PV,PT,&
   &  PRSFC=PFLUX%PFPLCL(:,KLEV,IBL),PRSFL=PFLUX%PFPLSL(:,KLEV,IBL),&
   &  PSLRFL=PFLUX%PFRTH(:,KLEV,IBL), PSSFC=PFLUX%PFPLCN(:,KLEV,IBL), PSSFL=PFLUX%PFPLSN(:,KLEV,IBL),&
   &  PCVL=PSURF5PCVL(:,IBL), PCVH=PSURF%PCVH(:,IBL), PCUR=PSURF%PCUR(:,IBL), &
   &  PWLMX=PSURFL%ZWLMX(:,IBL) , PEVAPSNW=PSURFL%ZEVAPSNW(:,IBL),&
   &  PUSRF=STATE%PU(:,KLEV,IBL),PVSRF=STATE%PV(:,KLEV,IBL),PTSRF=STATE%PT(:,KLEV,IBL),&
...
!-KPP
 ...
 !    & PSO0     ,PADVT    ,PADVS    ,PTRI0    ,PTRI1     ,&   !KPP
 !    & PSWDK_SAVE,PUSTRC  ,PVSTRC                        ,&   !KPP
   &  PZO=PSURF%PZO(:,:,IBL),PHO=PSURF%PHO(:,:,IBL),&
   &  PHO_INV=PSURF%PHO_INV(:,:,IBL),PDO=PSURF%PDO(:,:,IBL),&
   &  POCDEPTH=PSURF%POCDEP(:,IBL),&
   &  PUO0=PSURF%PUO0(:,:,IBL),PVO0=PSURF%PVO0(:,:,IBL),&
   &  PUOC=PSURF%PUOC(:,:,IBL),PVOC=PSURF%PVOC(:,:,IBL),&
   &  PTO0=PSURF%PTO0(:,:,IBL),&
   &  PSO0=PSURF%PSO0(:,:,IBL),PADVT=PSURF%PADVT(:,:,IBL),&
   &  PADVS=PSURF%PADVS(:,:,IBL),PTRI0=PSURF%PTRI0(:,:,IBL),&
   &  PTRI1=PSURF%PTRI1(:,:,IBL),&
   &  PSWDK_SAVE=PSURF%PSWDK(:,:,IBL),PUSTRC=PSURF%PUSTRC(:,IBL),&
   &  PVSTRC=PSURF%PVSTRC(:,IBL),&

 !    & PUSTOKES ,PVSTOKES ,PTAUOCX  ,PTAUOCY  ,PPHIOC    ,& ! Ocean TKE
 !    & PWSEMEAN ,PWSFMEAN ,                               & ! Ocean TKE
   &  PUSTOKES=PSURF%PUSTOKES(:,IBL),PVSTOKES=PSURF%PVSTOKES(:,IBL),&
   &  PTAUOCX=PSURF%PTAUOCX(:,IBL),PTAUOCY=PSURF%PTAUOCY(:,IBL),&
   &  PPHIOC=PSURF%PPHIOC(:,IBL),&
   &  PWSEMEAN=PSURF%PEMEAN(:,IBL),PWSFMEAN=PSURF%PFMEAN(:,IBL),&
 !    & LNEMOICETHK, ZTHKICE, &
...
!-DIAGNOSTICS OUTPUT
 ...
 !    & PBLOSSVT, PBGAINVT, &                                                  !CTESSEL
 !    & PLAI, PBIOM, PBLOSS, PBGAIN, PBIOMSTR, PBIOMSTR2, &                    !CTESSEL 
   & PTSDFL=PFLUX%PTSDFL(:,IBL), PROFD=PFLUX%PFWROD(:,IBL), PROFS=PFLUX%PFWRO1(:,IBL),&
   & PWFSD=PFLUX%PWFSD(:,IBL), PMELT=PFLUX%PFMLT(:,IBL), PFWEV=PFLUX%PFWEV(:,IBL) , PENES=PFLUX%PENES(:,:,IBL),&
   & PDIFM=PSURF%PDIFM(:,:,IBL), PDIFT=PSURF%PDIFT(:,:,IBL),&
   & PDIFS=PSURF%PDIFS(:,:,IBL), POTKE=PSURF%POTKE(:,:,IBL),&
   & PRESPBSTR=ZRESPBSTR(:,:,IBL),PRESPBSTR2=ZRESPBSTR2(:,:,IBL),PBIOMASS_LAST=ZBIOMASS_LAST(:,:,IBL),& !CTESSEL
   & PBIOMASSTR_LAST=ZBIOMASSTR_LAST(:,:,IBL),PBIOMASSTR2_LAST=ZBIOMASSTR2_LAST(:,:,IBL), &    !CTESSEL
   & PBLOSSVT=ZBLOSSVT(:,:,IBL), PBGAINVT=ZBGAINVT(:,:,IBL),&                                  !CTESSEL
   & PLAI=ZLAI(:,IBL), PBIOM=ZBIOM(:,IBL), PBLOSS=ZBLOSS(:,IBL),PBGAIN=ZBGAIN(:,IBL),&                   !CTESSEL
   & PBIOMSTR=ZBIOMSTR(:,IBL),PBIOMSTR2=ZBIOMSTR2(:,IBL),&    
                             !CTESSEL  
!-TENDENCIES OUTPUT
!     & PSNSE1  , PTSNE1 , PASNE1,&
!     & PRSNE1  , PWSNE1 , PTSAE1 , PTIAE1,&
!     & PWLE1   , PWSAE1,&
   &  PSNE1=PSURF_PSNSE1(:,:,IBL), PTSNE1=PSURF_PTSNE1(:,:,IBL), PASNE1=PSURF_PASNE1(:,1,IBL),&
...
!    & PLAIE1, PBSTRE1, PBSTR2E1, &                           !CTESSEL
   ...
   & PLAIE1=ZLAIE1(:,:,IBL),PBSTRE1=ZBSTRE1(:,:,IBL),PBSTR2E1=ZBSTR2E1(:,:,IBL), &                      !CTESSEL  
...
!     & PDHBIOS,PDHVEGS)                                        !CTESSEL 
   ...
   & PDHBIOS=ZDHBIOS(:,:,:,IBL),PDHVEGS=PSURFL%ZDHVEGS(:,:,:,IBL))     
