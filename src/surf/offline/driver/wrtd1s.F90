...
!     ------------------------------------------------------------------
     ...
     &            NPOSVEG ,NPOSCO2  
      ...
      USE YOMDPHY, ONLY : NTILES, NLON, NLEV
      USE YOMGDI1S , ONLY : &
...
!   time stuff
     ...
     &     ,D1T2M2(JL,1,1),D1D2M2(JL,1,1)&
     &     ,100.0_JPRB*FOEEWMO(D1D2M2(JL,1,1))/FOEEWMO(D1T2M2(JL,1,1))
...
!             -----------------------
     ...
     &          NPOSDTI5, NPOSDTI6, NPOSDTI7, NPOSDTI8,NPOSDTI9/) ! ENDUTRA 
      LDSEA=(D1STIFR(JL,1,1)+D1STIFR(JL,2,1)) >= 0.5_JPRB
      LDLAND=.NOT. LDSEA
     ...
     &      zjul,IYYMD,IHM,NSTEP&
     &     ,D1STIFR(JL,JTI,1),D1STITK(JL,JTI,1)&
     &     ,D1STIALB(JL,JTI,1),D1STIEVAP2(JL,JTI,IA,1)*ZMM&
...
!             ----------------------------------
     ...
     &     ,(D1STIFR(JL,5,1)*D1STIH2(JL,5,IA,1)+&
     &      D1STIFR(JL,7,1)*D1STIH2(JL,7,IA,1))*ZWA&
     &     ,(D1STIFR(JL,5,1)*D1STILE2(JL,5,IA,1)+&
     &      D1STIFR(JL,7,1)*D1STILE2(JL,7,IA,1))*ZWA&
...
! WRITE(*,*)   'ZWA=',ZWA, '  ZMM=',ZMM,' IA=',IA
 ...
 WRITE(NPOSCO2,'(F10.3,1X,I8,1X,I4,1X,I8,35(1X,E14.6E3))')&
     ...
     &      ,D1SLAI2(JL,IA,1)*ZWA,        D1SBIOM2(JL,IA,1)*ZWA        &
     &      ,D1SBLOSS2(JL,IA,1)*ZWA,      D1SBGAIN2(JL,IA,1)*ZWA       &
     &      ,D1SBIOMSTR2(JL,IA,1)*ZWA,    D1SBIOMSTRB2(JL,IA,1)*ZWA    &
     &      ,D1SVTLAI2(JL,1,IA,1)*ZWA,    D1SVTLAI2(JL,2,IA,1)*ZWA     &
     &      ,D1SVTBIOM2(JL,1,IA,1)*ZWA,   D1SVTBIOM2(JL,2,IA,1)*ZWA    &
     &      ,D1SVTBLOSS2(JL,1,IA,1)*ZWA,  D1SVTBLOSS2(JL,2,IA,1)*ZWA   &
     &      ,D1SVTBGAIN2(JL,1,IA,1)*ZWA,  D1SVTBGAIN2(JL,2,IA,1)*ZWA   &
     &      ,D1SVTBIOMSTR2(JL,1,IA,1)*ZWA,D1SVTBIOMSTR2(JL,2,IA,1)*ZWA &
     &      ,D1SVTBIOMSTRB2(JL,1,IA,1)*ZWA,D1SVTBIOMSTRB2(JL,2,IA,1)*ZWA &
...
!             ------------------------------------------------
 ...
 WRITE(NPOSVEG,'(F10.3,1X,I8,1X,I4,1X,I8,15(1X,E12.6))')&
     ...
     &      ,D1SVTF22(JL,1,IA,1)*ZWA,D1SVTF22(JL,2,IA,1)*ZWA &
     &      ,D1SVTLE2(JL,1,IA,1)*ZWA,D1SVTLE2(JL,2,IA,1)*ZWA &
     &      ,D1SVTFR2(JL,1,IA,1)*ZWA,D1SVTFR2(JL,2,IA,1)*ZWA &
     &      ,D1SVTDS2(JL,1,IA,1)*ZWA,D1SVTDS2(JL,2,IA,1)*ZWA &
     &      ,D1SVTDMAX2(JL,1,IA,1)*ZWA,D1SVTDMAX2(JL,2,IA,1)*ZWA &
! fractions
           &,D1SWFR2(JL,IA,1)*ZWA 
...
REAL(KIND=JPRB), PARAMETER :: R=1.380658_JPRB*6.0221367_JPRB
REAL(KIND=JPRB), PARAMETER :: RD=1000._JPRB*R/28.9644_JPRB
REAL(KIND=JPRB), PARAMETER :: RV=1000._JPRB*R/18.0153_JPRB
