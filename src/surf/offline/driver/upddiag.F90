...
DO JL=KIDIA,KFDIA
  ...
  D1SWDS(JLK,IBL)=PTSPHY*RHOH2O*DOT_PRODUCT(PWSAE1(JL,:),RDAW(:))
  DO JK=1,KLEVS
    D1SSDSSL(JLK,JK,IBL)=PTSPHY*RHOH2O*PWSAE1(JL,JK)*RDAW(JK)
  ENDDO
ENDDO
...
! Tile energy fluxes
D1STISRD(KIDIAF:KFDIAF,:,IBL) = PDHTLS(KIDIA:KFDIA,:,4)
D1STISRU(KIDIAF:KFDIAF,:,IBL) = PDHTLS(KIDIA:KFDIA,:,5)
D1STITRD(KIDIAF:KFDIAF,:,IBL) = PDHTLS(KIDIA:KFDIA,:,6)
D1STITRU(KIDIAF:KFDIAF,:,IBL) = PDHTLS(KIDIA:KFDIA,:,7)
D1STIH(KIDIAF:KFDIAF,:,IBL)   = PDHTLS(KIDIA:KFDIA,:,8)
D1STILE(KIDIAF:KFDIAF,:,IBL)  = PDHTLS(KIDIA:KFDIA,:,9)
D1STIGFL(KIDIAF:KFDIAF,:,IBL) = PDHTLS(KIDIA:KFDIA,:,10)
D1STII(KIDIAF:KFDIAF,:,IBL) = 0._JPRB
D1STIEVAP(KIDIAF:KFDIAF,:,IBL) = PDHTLS(KIDIA:KFDIA,:,11)
D1STISKC(KIDIAF:KFDIAF,:,IBL) = PDHTLS(KIDIA:KFDIA,:,12)
D1STIEVAPU(KIDIAF:KFDIAF,:,IBL) = PEVAPTIU(KIDIA:KFDIA,:)

! Average energy fluxes
D1SRFLD(KIDIAF:KFDIAF,IBL) = PSRSOD(KIDIA:KFDIA)
D1SRFLU(KIDIAF:KFDIAF,IBL) = PFRSO(KIDIA:KFDIA,KLEV)-PSRSOD(KIDIA:KFDIA)
D1TRFLD(KIDIAF:KFDIAF,IBL) = PSRTHD(KIDIA:KFDIA)
D1TRFLU(KIDIAF:KFDIAF,IBL) = PFRTH(KIDIA:KFDIA,KLEV)-PSRTHD(KIDIA:KFDIA)
D1AHFS(KIDIAF:KFDIAF,IBL)  = PDIFTS(KIDIA:KFDIA,KLEV)
D1AHFL(KIDIAF:KFDIAF,IBL)  = PFTLHEV(KIDIA:KFDIA)+PFTLHSB(KIDIA:KFDIA)
D1MOMU(KIDIAF:KFDIAF,IBL)  = PSTRTU(KIDIA:KFDIA,KLEV)
D1MOMV(KIDIAF:KFDIAF,IBL)  = PSTRTV(KIDIA:KFDIA,KLEV)
! Snow energy fluxes
D1STNSRD(KIDIAF:KFDIAF,IBL) = PDHTSS(KIDIA:KFDIA,1,7)
D1STNSRU(KIDIAF:KFDIAF,IBL) = PDHTSS(KIDIA:KFDIA,1,8)
D1STNTRD(KIDIAF:KFDIAF,IBL) = PDHTSS(KIDIA:KFDIA,1,9)
D1STNTRU(KIDIAF:KFDIAF,IBL) = PDHTSS(KIDIA:KFDIA,1,10)
D1STNGFL(KIDIAF:KFDIAF,IBL) = PDHTSS(KIDIA:KFDIA,1,13)
D1STNM(KIDIAF:KFDIAF,IBL)   = PDHTSS(KIDIA:KFDIA,1,14)
D1HFLUXRF(KIDIAF:KFDIAF,IBL) = 0._JPRB ! PDHTSS(KIDIA:KFDIA,1,16) Not available in surf 
! Soil energy fluxes
D1ST1SRD(KIDIAF:KFDIAF,IBL) = PDHTTS(KIDIA:KFDIA,1,5)
D1ST1SRU(KIDIAF:KFDIAF,IBL) = PDHTTS(KIDIA:KFDIA,1,6)
D1ST1TRD(KIDIAF:KFDIAF,IBL) = PDHTTS(KIDIA:KFDIA,1,7)
D1ST1TRU(KIDIAF:KFDIAF,IBL) = PDHTTS(KIDIA:KFDIA,1,8)
D1ST1H(KIDIAF:KFDIAF,IBL)   = PDHTTS(KIDIA:KFDIA,1,9)
D1ST1LE(KIDIAF:KFDIAF,IBL)  = PDHTTS(KIDIA:KFDIA,1,10)
D1STAGFL(KIDIAF:KFDIAF,:,IBL) = PDHTTS(KIDIA:KFDIA,:,11)
D1STASF(KIDIAF:KFDIAF,:,IBL)  = PDHTTS(KIDIA:KFDIA,:,12)
! Snow water fluxes
D1SSFL(KIDIAF:KFDIAF,IBL)  = PDHSSS(KIDIA:KFDIA,1,2)
D1SSFC(KIDIAF:KFDIAF,IBL)  = PDHSSS(KIDIA:KFDIA,1,3)
D1SWNJQ(KIDIAF:KFDIAF,IBL) = PDHSSS(KIDIA:KFDIA,1,4)
D1SWNM(KIDIAF:KFDIAF,IBL)  = PDHSSS(KIDIA:KFDIA,1,5)
D1WFLUXRF(KIDIAF:KFDIAF,IBL) = 0._JPRB ! PDHSSS(KIDIA:KFDIA,1,6) Not available in surf 
D1WSN(KIDIAF:KFDIAF,IBL) = 0._JPRB ! PDHSSS(KIDIA:KFDIA,1,7) Not available in surf 
! Interception water fluxes
D1SWLIT(KIDIAF:KFDIAF,IBL) = PDHIIS(KIDIA:KFDIA,2)+PDHIIS(KIDIA:KFDIA,3)
D1SWLJQ(KIDIAF:KFDIAF,IBL) = PDHIIS(KIDIA:KFDIA,4)
!write(6,*) 'upddiag chech interceptie balans'
!write(6,*) 'interceptie',D1SWLIT
!write(6,*) 'evaporatie',D1SWLjq
! Soil water fluxes
D1SW1JBG(KIDIAF:KFDIAF,IBL) = PDHWLS(KIDIA:KFDIA,1,9) ! bare ground evaporation 
                                                      ! with mismatches from 
                                                      ! intercep & snow tiles
D1SW1TF(KIDIAF:KFDIAF,IBL)  = PDHWLS(KIDIA:KFDIA,1,3)+PDHWLS(KIDIA:KFDIA,1,4)
D1SWAGFL(KIDIAF:KFDIAF,:,IBL) = PDHWLS(KIDIA:KFDIA,:,8)

D1SW1RI(KIDIAF:KFDIAF,IBL) = PDHWLS(KIDIA:KFDIA,1,6) ! surface runoff
D1SWARS(KIDIAF:KFDIAF,:,IBL) = 0._JPRB ! excess runoff
D1SWAEXT(KIDIAF:KFDIAF,:,IBL) = PDHWLS(KIDIA:KFDIA,:,7)
D1SW1M(KIDIAF:KFDIAF,IBL) = PDHSSS(KIDIA:KFDIA,1,5)
D1SW1C(KIDIAF:KFDIAF,IBL) = PDHWLS(KIDIA:KFDIA,1,10)
D1SWAC(KIDIAF:KFDIAF,:,IBL) = 0._JPRB ! clipping
...
D1SVTAG(KIDIAF:KFDIAF,:,IBL) =        PDHCO2S(KIDIA:KFDIA,:,1)
D1SVTRD(KIDIAF:KFDIAF,:,IBL) =        PDHCO2S(KIDIA:KFDIA,:,2)
D1SVTAN(KIDIAF:KFDIAF,:,IBL) =        PDHCO2S(KIDIA:KFDIA,:,3)
D1SVTRSOIL_STR(KIDIAF:KFDIAF,:,IBL) = PDHCO2S(KIDIA:KFDIA,:,4)
D1SVTRECO(KIDIAF:KFDIAF,:,IBL) =      PDHCO2S(KIDIA:KFDIA,:,5)
D1SVTCO2FLUX(KIDIAF:KFDIAF,:,IBL) =   PDHCO2S(KIDIA:KFDIA,:,6)
D1SVTRNOQ10(KIDIAF:KFDIAF,:,IBL) =    PDHCO2S(KIDIA:KFDIA,:,7)
! CO2 flux per grid
D1SAN(KIDIAF:KFDIAF,IBL)=PAN(KIDIA:KFDIA)
D1SAG(KIDIAF:KFDIAF,IBL)=PAG(KIDIA:KFDIA)
D1SRD(KIDIAF:KFDIAF,IBL)=PRD(KIDIA:KFDIA)
D1SRSOIL_STR(KIDIAF:KFDIAF,IBL)=PRSOIL_STR(KIDIA:KFDIA)
D1SRECO(KIDIAF:KFDIAF,IBL)=PRECO(KIDIA:KFDIA)
D1SCO2FLUX(KIDIAF:KFDIAF,IBL)=PCO2FLUX(KIDIA:KFDIA)
D1SCH4FLUX(KIDIAF:KFDIAF,IBL)=PCH4FLUX(KIDIA:KFDIA)
D1SBVOCFLUX(KIDIAF:KFDIAF,IBL)=PBVOCFLUX(KIDIA:KFDIA,1)

! Biomass per vegetation type
D1SVTBIOM(KIDIAF:KFDIAF,:,IBL) = PDHBIOS(KIDIA:KFDIA,:,1)
D1SVTBLOSS(KIDIAF:KFDIAF,:,IBL) = PDHBIOS(KIDIA:KFDIA,:,2)
D1SVTBGAIN(KIDIAF:KFDIAF,:,IBL) = PDHBIOS(KIDIA:KFDIA,:,3) 
D1SVTBIOMSTR(KIDIAF:KFDIAF,:,IBL) = PDHBIOS(KIDIA:KFDIA,:,4)
D1SVTBIOMSTRB(KIDIAF:KFDIAF,:,IBL) = PDHBIOS(KIDIA:KFDIA,:,5)
! Biomass per grid
D1SBIOM(KIDIAF:KFDIAF,IBL)=PBIOM(KIDIA:KFDIA)
D1SBLOSS(KIDIAF:KFDIAF,IBL)=PBLOSS(KIDIA:KFDIA)
D1SBGAIN(KIDIAF:KFDIAF,IBL)=PBGAIN(KIDIA:KFDIA)
D1SBIOMSTR(KIDIAF:KFDIAF,IBL)=PBIOMSTR(KIDIA:KFDIA)
D1SBIOMSTRB(KIDIAF:KFDIAF,IBL)=PBIOMSTR2(KIDIA:KFDIA)

! LAI per grid
D1SLAI(KIDIAF:KFDIAF,IBL)=PLAI(KIDIA:KFDIA)

! Variables per vegetation type
D1SVTFR(KIDIAF:KFDIAF,:,IBL) = PDHVEGS(KIDIA:KFDIA,:,1)
D1SVTLAI(KIDIAF:KFDIAF,:,IBL) = PDHVEGS(KIDIA:KFDIA,:,2)
D1SVTGC(KIDIAF:KFDIAF,:,IBL) = PDHVEGS(KIDIA:KFDIA,:,3)
D1SVTGA(KIDIAF:KFDIAF,:,IBL) = PDHVEGS(KIDIA:KFDIA,:,4)
D1SVTF2(KIDIAF:KFDIAF,:,IBL) = PDHVEGS(KIDIA:KFDIA,:,5)
D1SVTDS(KIDIAF:KFDIAF,:,IBL) = PDHVEGS(KIDIA:KFDIA,:,6)
D1SVTDMAX(KIDIAF:KFDIAF,:,IBL) = PDHVEGS(KIDIA:KFDIA,:,7)
D1SVTLE(KIDIAF:KFDIAF,:,IBL) = PDHVEGS(KIDIA:KFDIA,:,8)

! Tile values
D1STIFR(KIDIAF:KFDIAF,:,IBL)  = PDHTLS(KIDIA:KFDIA,:,1)
D1STITK(KIDIAF:KFDIAF,:,IBL)  = PDHTLS(KIDIA:KFDIA,:,2)
D1STIALB(KIDIAF:KFDIAF,:,IBL) = PDHTLS(KIDIA:KFDIA,:,3)
!AuxSN
D1SNDEPTH(KIDIAF:KFDIAF,IBL) = PDHSSS(KIDIA:KFDIA,1,1)
D1SNFR(KIDIAF:KFDIAF,IBL) = PDHTLS(KIDIA:KFDIA,5,1)+PDHTLS(KIDIA:KFDIA,7,1)
D1SNTFR(KIDIAF:KFDIAF,IBL) = D1SNFR(KIDIAF:KFDIAF,IBL)*PDHTSS(KIDIA:KFDIA,1,2)
D1SNAFR(KIDIAF:KFDIAF,IBL) = D1SNFR(KIDIAF:KFDIAF,IBL)*PDHTSS(KIDIA:KFDIA,1,6)
!AuxW
WHERE (PDHTLS(KIDIA:KFDIA,3,1) > ZEPSILON )
  D1SWFR(KIDIAF:KFDIAF,IBL) = PDHTLS(KIDIA:KFDIA,3,1)/MAX(ZEPSILON, 1._JPRB-(PDHTLS(KIDIA:KFDIA,5,1)+PDHTLS(KIDIA:KFDIA,7,1)))
ELSEWHERE
  D1SWFR(KIDIAF:KFDIAF,IBL) = 0._JPRB
ENDWHERE
D1SWAFR(KIDIAF:KFDIAF,:,IBL) = PDHWLS(KIDIA:KFDIA,:,1)-PDHWLS(KIDIA:KFDIA,:,2) ! liquid water (kg m-2)
!AuxT
D1SAVTK(KIDIAF:KFDIAF,IBL) = 0._JPRB
DO JTILE=1,KTILES
  D1SAVTK(KIDIAF:KFDIAF,IBL)=D1SAVTK(KIDIAF:KFDIAF,IBL)+PDHTLS(KIDIA:KFDIA,JTILE,1)*PDHTLS(KIDIA:KFDIA,JTILE,2)
ENDDO
D1SVEGTK(KIDIAF:KFDIAF,IBL) = (MAX(ZEPS,PDHTLS(KIDIA:KFDIA,4,1))*PDHTLS(KIDIA:KFDIA,4, 2)+ &
     &      PDHTLS(KIDIA:KFDIA,6,1)*PDHTLS(KIDIA:KFDIA,6,2))/  &
     &     (MAX(ZEPS,PDHTLS(KIDIA:KFDIA,4,1))+PDHTLS(KIDIA:KFDIA,6,1))
D1SGHF(KIDIAF:KFDIAF,IBL) = PDHTTS(KIDIA:KFDIA,1,14)
D1SRADTK(KIDIAF:KFDIAF,IBL) = 0._JPRB! not calculated in vdfmain
D1SRADTK(KIDIAF:KFDIAF,IBL) =(PDHTLS(KIDIA:KFDIA,1,1)*(PDHTLS(KIDIA:KFDIA,1, 2))**4+ &
...
ENDIF
D1SALB(KIDIAF:KFDIAF,IBL) = PALB(KIDIA:KFDIA)
D1SVEGEV(KIDIAF:KFDIAF,IBL) = 0._JPRB! not used
D1SDSH(KIDIAF:KFDIAF,IBL) = 0._JPRB! DelSoilHeat of skin layer (0)
D1STNDH(KIDIAF:KFDIAF,IBL) = PDHTSS(KIDIA:KFDIA,1,15)     ! DelColdCont

D1SSDS(KIDIAF:KFDIAF,IBL)=PTSPHY*PSNSE1(KIDIA:KFDIA,1)
D1SWLDS(KIDIAF:KFDIAF,IBL)=PTSPHY*PWLE1(KIDIA:KFDIA)
!write(6,*) 'delint',D1SWLDS

! Water budgets
D1SLSRF(KIDIAF:KFDIAF,IBL) = PLSRF(KIDIA:KFDIA)
D1SCRF(KIDIAF:KFDIAF,IBL) = PCRF(KIDIA:KFDIA)
D1SLSSF(KIDIAF:KFDIAF,IBL) = PLSSF(KIDIA:KFDIA) 
D1SCSF(KIDIAF:KFDIAF,IBL) = PCSF(KIDIA:KFDIA)
D1STE(KIDIAF:KFDIAF,IBL) = PDIFTQ(KIDIA:KFDIA,KLEV)
D1STSRO(KIDIAF:KFDIAF,IBL) = PFWRO1(KIDIA:KFDIA)
D1STRO(KIDIAF:KFDIAF,IBL) = PFWRO1(KIDIA:KFDIA) + PFWROD(KIDIA:KFDIA)
D1SMLT(KIDIAF:KFDIAF,IBL) = PFWMLT(KIDIA:KFDIA)

! Screen level 
D1T2M(KIDIAF:KFDIAF,IBL) = PT2M(KIDIA:KFDIA)
D1D2M(KIDIAF:KFDIAF,IBL) = PD2M(KIDIA:KFDIA)
! forcing (end of timestep)
D1STAIR(KIDIAF:KFDIAF,IBL) = PTAIR(KIDIA:KFDIA)
D1SQAIR(KIDIAF:KFDIAF,IBL) = PQAIR(KIDIA:KFDIA)
D1SUWIND(KIDIAF:KFDIAF, IBL) = PUWIND(KIDIA:KFDIA)
D1SVWIND(KIDIAF:KFDIAF, IBL) = PVWIND(KIDIA:KFDIA)
D1SPSURF(KIDIAF:KFDIAF, IBL) = PPSURF(KIDIA:KFDIA)

! Biogenic VOC emission diagnostics per vegetation type. Check how this is computed!
D1SBVOCST1(KIDIAF:KFDIAF,:,IBL) = PDHBVOCS(KIDIA:KFDIA,:,1)
D1SBVOCST2(KIDIAF:KFDIAF,:,IBL) = PDHBVOCS(KIDIA:KFDIA,:,2)
