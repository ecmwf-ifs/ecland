...
CFORCU=TRIM(CFORCU)
WRITE(NULOUT,*)'Inquiring U wind forcing  ','CFORCU= ',CFORCU, 'NCNOWRIT= ',NCNOWRIT


IF( MYPROC == 1 ) THEN
 NCID = NCOPN(CFORCU, NCNOWRIT, IERR)
 WRITE(NULOUT,*)'IERR for CFORCU =',IERR
 WRITE(NULOUT,*)'NETCDF-FILE U Forcing OPENED ON UNIT ',NCID
...
WRITE(NULOUT,*)'closed U wind forcing  '
WRITE(NULOUT,*)'CNAME(1:6)= ',CNAME(1:6)
WRITE(NULOUT,*)'LWIND2D= ',LWIND2D
...
! read surface geopotential        
  ...
  IF(LOINTP)THEN
    ...
    WRITE(NULOUT,*)'LOINTP option not possible in sufcdf'
    CALL ABOR1('SUFCDF:')
  ELSE
