SUBROUTINE CALLPAR1S (CDCONF &
     & , KIDIA,KFDIA,KLON,IBL,KLEVS,KTILES,KVTYPES &
     ...
     & , KDHVCO2S,KDHFCO2S &
     & , KDHVBIOS,KDHFBIOS,KDHVVEGS,KDHFVEGS &
     & , PTSPHY &
!-----------------------------------------------------------------------
! - INPUT .
     & , PGEOM1 , PAPRS &
...
USE YOEPHY   , ONLY : LERADS, LESICE, LESURF, LEVDIF, LECTESSEL, LEINTWIND, LEWARMSTART, LESNML, NALBEDOSCHEME
...
REAL(KIND=JPRB) :: PTVL(KLON)  ,PCO2TYP(KLON), PTVH(KLON)  ,PCVL(KLON)  ,PCVH(KLON) ,PCUR(KLON)
REAL(KIND=JPRB) :: PLAILC(KLON),PLAIHC(KLON)
REAL(KIND=JPRB) :: PSOTY(KLON) ,PSDOR(KLON)
REAL(KIND=JPRB) :: PCI(KLON)   ,PSST(KLON)
...
LOGICAL LLLAND(KLON),LLSICE(KLON),LLNH(KLON)
...
CALL SURFBC(YDSURF, KIDIA,KFDIA,KLON,KTILES,KLEVSN &
     &           ,PTVL   ,PCO2TYP,PTVH   ,PSOTY  ,PSDOR,PCVL   ,PCVH  ,PCUR &
     &           ,PLAILC ,PLAIHC, PLAILI, PLAIHI &     
     &           ,PLSM   ,PCI    ,PCLAKE,PHLICE,PGEMU  ,PSNS, PWL,PRSN &  
     &           ,LLLAND ,LLSICE ,LLLAKE,LLNH, LLOCN_KPP &    
     &           ,ITVL   ,ICO2TYP, ITVH   ,ISOTY &
     &           ,ZCVL   ,ZCVH   ,ZCUR  ,ZLAIL , ZLAIH,ZWLMX  ,ZFRTI)
...
!* get time variables
    ...
    IYY=NCCAA(IYMD)
    IINC=0
    ISEC0=MOD(NSSSSS+NINT(RSTATI),NINT(RDAY))   
...
!-END OF CALL SURFTSTP
  CALL SURFTSTP( YDSURF,KIDIA   , KFDIA , KLON  , KLEVS, KTILES,&
     ...
     & GELAT(:,IBL), PANFMVT, PANDAYVT, PMU0M,&                      !CTESSEL
