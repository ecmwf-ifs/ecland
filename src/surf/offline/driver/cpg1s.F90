...
! replace SST with the top layer temp. of the ocean mixed layer (~ 1m depth)
    VFSST(IST:IEND) = TONU0(1:IPROMA,1,IBL) + RTT ! The unit of PSST(VFSST) is K.  
  ENDIF
...
!            -------------
  ...
  CALL CALLPAR1S (CDCONF &
     & , 1, IPROMA   , IPROMA , NCSS  , NTILES, NVHILO & 
...
! - INPUT .
     ...
     & , HLICENU0(1:IPROMA,IBL),  HLMLNU0(1:IPROMA,IBL), VFLDEPTH(IST:IEND), VFCLAKEF(IST:IEND) &   ! FLAKE 
     & , VFALBF(IST:IEND) &
     ...
     & , VFCVL(IST:IEND)  , VFCVH(IST:IEND)  , VFCUR(IST:IEND) , VFTVL(IST:IEND)  , VFCO2TYP(IST:IEND), VFISOP_EP(IST:IEND), VFTVH(IST:IEND)  &
     & , VFLAIL(IST:IEND) , VFLAIH(IST:IEND) , VFFWET(IST:IEND) , VFAVGPAR(IST:IEND), VFRSML(IST:IEND) , VFRSMH(IST:IEND) &
     & , VFSOTY(IST:IEND) , VFSDOR(IST:IEND) &
     & , VFCI(IST:IEND)   , VFCIL(IST:IEND), VFSST(IST:IEND)   , GEMU(IST:IEND),  GELAT(IST:IEND), ZCFLX(1:IPROMA,:,IBL) &
     ...
     & , VDRESPBSTR(IST:IEND,:),VDRESPBSTR2(IST:IEND,:),VDBIOMASS_LAST(IST:IEND,:),BSTRNU0(1:IPROMA,:,IBL),BSTR2NU0(1:IPROMA,:,IBL) & 
