...
REAL(KIND=JPRB) ::  ZAN(NPOI),ZAG(NPOI),ZRD(NPOI) &
     &   , ZRSOIL_STR(NPOI),ZRECO(NPOI),ZCO2FLUX(NPOI),ZCH4FLUX(NPOI),ZBVOCFLUX(NPOI,NDHVBVOCS)
...
REAL(KIND=JPRB) :: ZDHBVOCS(NPOI,NVHILO,NDHVBVOCS)
...
! - INPUT .
     ...
     & , HLICENU0(1:IPROMA,IBL),  HLMLNU0(1:IPROMA,IBL), VFLDEPTH(IST:IEND,IBL), VFCLAKEF(1:IPROMA,IBL) &   ! FLAKE 
     & , VFALBF(1:IPROMA,IBL) &
     ...
     & , VFCVL(1:IPROMA,IBL)  , VFCVH(1:IPROMA,IBL)  , VFCUR(1:IPROMA,IBL) , VFTVL(1:IPROMA,IBL)  , VFCO2TYP(1:IPROMA,IBL), VFISOP_EP(IST:IEND,IBL), VFTVH(1:IPROMA,IBL)  &
     & , VFLAIL(1:IPROMA,IBL) , VFLAIH(1:IPROMA,IBL) , VFFWET(1:IPROMA,IBL) , VFAVGPAR(IST:IEND,IBL), VFRSML(1:IPROMA,IBL) , VFRSMH(1:IPROMA,IBL) &
     & , VFSOTY(1:IPROMA,IBL) , VFSDOR(1:IPROMA,IBL) &
     & , VFCI(1:IPROMA,IBL)   , VFCIL(1:IPROMA,IBL), VFSST(1:IPROMA,IBL)   , GEMU(1:IPROMA,IBL),  GELAT(1:IPROMA,IBL), ZCFLX(1:IPROMA,:,IBL) &
...
! - OUTPUT .
     ...
     & , ZDHCO2S(IST:IEND,:,:),ZDHBVOCS(IST:IEND,:,:),ZDHBIOS(IST:IEND,:,:),ZDHVEGS(IST:IEND,:,:))
...
!     ------------------------------------------------------------------
  ...
  CALL UPDDIAG( &
   ...
   & ZEVAPTIU(IST:IEND,:),&
   & ZAN(IST:IEND),ZAG(IST:IEND),ZRD(IST:IEND),ZRSOIL_STR(IST:IEND),ZRECO(IST:IEND),ZCO2FLUX(IST:IEND),ZCH4FLUX(IST:IEND), ZBVOCFLUX(IST:IEND,:), &
   ...
   & ZFRSO(IST:IEND,:),FSSRD(1:IPROMA,IBL),ZFRTH(IST:IEND,:),FSTRD(1:IPROMA,IBL),VDALB(1:IPROMA,IBL),ZFTLHEV(IST:IEND),ZFTLHSB(IST:IEND), &
   & ZDIFTQ(IST:IEND,:),ZDIFTS(IST:IEND,:),ZSTRTU(IST:IEND,:),ZSTRTV(IST:IEND,:),ZWSAE1(IST:IEND,:),ZSNSE1(IST:IEND,:), &
       & ZWLE1(IST:IEND), &
   & FCRF(1:IPROMA,IBL)    ,FCSF(1:IPROMA,IBL)   ,FLSRF(1:IPROMA,IBL),FLSSF(1:IPROMA,IBL), ZFWRO1(IST:IEND),ZFWROD(IST:IEND),ZFWMLT(IST:IEND),&
