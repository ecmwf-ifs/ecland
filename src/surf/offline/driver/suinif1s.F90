...
!DO IST=1,NPOI,NPROMA
!  IEND=MIN(IST+NPROMA-1,NPOI)
  CALL SUSDP_DFLT(YDSURF=YDSURF,PTVL=VFTVL(IST:IEND,IBL),PTVH=VFTVH(IST:IEND,IBL),PSLT=VFSOTY(IST:IEND,IBL),&
...
DO IST=1,NPOI,NPROMA
  ...
  IST1 = 1
  IEND1 = IEND-IST+1
  CALL SUSDP_DERIV(YDSURF=YDSURF,KIDIA=IST1,KFDIA=IEND1,PSLT=VFSOTY(IST:IEND,IBL),&
          & PSSDP2=GPD_SDP2(IST:IEND,:),PSSDP3=GPD_SDP3(IST:IEND,:,:),KLEVS3D=NCSS)
ENDDO
!$OMP END PARALLEL DO
...
ALLOCATE (UNLEV0(NPROMA, NLEV, NBLOCKS))
ALLOCATE (VNLEV0(NPROMA, NLEV, NBLOCKS))
ALLOCATE (TNLEV0(NPROMA, NLEV, NBLOCKS))
ALLOCATE (QNLEV0(NPROMA, NLEV, NBLOCKS))
...
ALLOCATE (UNLEV1(NPROMA, NLEV, NBLOCKS))
ALLOCATE (VNLEV1(NPROMA, NLEV, NBLOCKS))
ALLOCATE (TNLEV1(NPROMA, NLEV, NBLOCKS))
ALLOCATE (QNLEV1(NPROMA, NLEV, NBLOCKS))
