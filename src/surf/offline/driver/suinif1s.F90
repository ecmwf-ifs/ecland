...
USE YOMDPHY  , ONLY : NPOI     ,NTRAC    ,NCSS, NLEV, YSURF, NBLOCKS        
...
DO IST=1,NPOI,NPROMA
  ...
  CALL SUSDP_DFLT(YSURF=YSURF,PTVL=VFTVL(IST:IEND),PTVH=VFTVH(IST:IEND),PSLT=VFSOTY(IST:IEND),&
...
DO IST=1,NPOI,NPROMA
  ...
  IST1 = 1
  IEND1 = IEND-IST+1
  CALL SUSDP_DERIV(YSURF=YSURF,KIDIA=IST1,KFDIA=IEND1,PSLT=VFSOTY(IST:IEND),&
          & PSSDP2=GPD_SDP2(IST:IEND,:),PSSDP3=GPD_SDP3(IST:IEND,:,:),KLEVS3D=NCSS)
ENDDO
!$OMP END PARALLEL DO
...
ALLOCATE (PNLP0(NPOI))
ALLOCATE (PNLP1(NPOI))
ALLOCATE (UNLEV1(NPOI, NLEV))
ALLOCATE (VNLEV1(NPOI, NLEV))
ALLOCATE (TNLEV1(NPOI, NLEV))
ALLOCATE (QNLEV1(NPOI, NLEV))
