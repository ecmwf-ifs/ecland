...
# - extract data from mars, cds or local disk. 
#   - Data must be in monthly chunks and in GRIB format -
# - forcing fields are: PSurf, Tair, Qair, Wind_E, Wind_N, SWdown, LWdown, Rainf, Snowf, Ctpf
# - preprocess forcing fields and create a continuous time series for each site
# - forcing fields are extracted for each site and concatenated over the year
#**************************************************************#
...
get_lsm() {
    ...
    if [[ $XDATA_DIR == *rdx* ]]; then
      ...
      wget -O "${lsmgrib}" $XDATA_DIR/lsmoro
      grib_copy -w shortName=lsm ${lsmgrib} ${lsmgrib}.tmp
      mv -f ${lsmgrib}.tmp ${lsmgrib}
    fi
}
